...
两个独立仓库变量控制故障切换。`DSH_CI_FAILOVER_LINUX=selfhosted` 只把三个必需 Linux 工作作业 `node-24`、`node-24-coverage`、`node-24-consumers`，以及 `all-checks-passed` 判定作业重定向到 `vm-backup` 池。`DSH_CI_FAILOVER_WINDOWS=selfhosted` 只把 `windows-native` 重定向到 `dsh-win-ci` 池。一个平台的故障不会重定向另一个平台。
...
具备写权限的协作者可以通过修改一个仓库变量并重新运行 CI 来恢复一个故障平台。Linux 与 Windows 故障保持独立，Fusion 验收和可移植必需作业继续运行在标准托管运行器上。
