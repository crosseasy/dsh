...
- After changing branches or updating a checkout that deleted packages, contributors can run `pnpm run clean` to remove stale package directories before rebuilding. `pnpm run constraints` ignores only the same generated residue so standalone quality gates stay reproducible without silently accepting unexpected files. Unknown files in a manifest-less package directory require manual classification instead of being deleted.
