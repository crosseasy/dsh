...
The rewrite classifies package references by parsed syntax and metadata instead of treating every quoted token as a package name. It rewrites module specifiers, `package.json` names and dependency keys, `peerDependenciesMeta` keys, `tsconfig*.json` `paths` keys, and Cordis Loader `name:` values. Identically spelled strings that are not package references stay unchanged: runtime event ids, locale and data ids, ordinary JSON values, quoted natural-language text, the `cordis.yml` config-file family, the Loader's literal `cordis:` builtin prefix (`cordis:include`, `cordis:group` — see `vendor/loader/src/config/tree.ts`), kind strings like `cordis-config-entry`, `@deepseek-ai/dsh-tool-cordis`, Schemastery's upstream `Symbol.for('schemastery')` and `vendor:` metadata field, and the `packages/<group>/` directory names in `GROUP_ORDER` (`scripts/gen-module-graph.ts`, `scripts/gen-doc-graphs.ts`).

YAML rewriting uses the repository-owned Loader-file policy: `*cordis*.yml` and `*cordis*.yaml`. The parser accepts the runtime `!!js` dialect, rejects malformed or unsupported documents, and edits exact scalar ranges only in top-level entries, id-targeted patch name guards, `insert` entries, group children, and include `config.patches`. The built-in `cordis:group` and `cordis:include` names remain unchanged while selecting traversal into their nested Loader entries. Existing scalar quotes and comments remain intact; a plain scalar whose scoped form begins with `@` becomes double-quoted. Reverse mode guarantees equivalent parsed Loader data and directional idempotence, not byte-identical restoration of that originally plain scalar.
...
Markdown uses each fence's declared language to classify module and package-metadata references. Valid JSON/JSONC fences rewrite dependency and `peerDependenciesMeta` keys while preserving ordinary values; malformed JSON/JSONC fences remain byte-identical. Generic YAML fences lack the filename or role metadata required to identify Loader input and remain unchanged. Prose under `docs/` rewrites explicit module syntax, while quoted natural-language text remains unchanged. Prose elsewhere is not eligible.
