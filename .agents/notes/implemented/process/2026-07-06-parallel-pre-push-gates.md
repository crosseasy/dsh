...
The per-gate package scripts remain available for ad hoc local runs. `hygiene` and `check:all` reuse the same artifact dependency graph: one build precedes `publint`, built-package invariants, and the NodeNext consumer, while independent source checks can start immediately. Standalone `hygiene` keeps the local four-worker cap and never cleans the worktree; `doc-sync` owns its member list in the scheduler ([doc-sync through the gate scheduler](../../archived/process/2026-07-21-doc-sync-through-gate-scheduler.md)).
...
Standalone hygiene produces its artifact inputs instead of trusting residual output. Rebuilding an already-built worktree costs time, but clean and already-built states execute the same graph; preserving the worktree lets workspace constraints continue to reject unexpected package directories.
