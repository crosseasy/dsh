...
一个编码 agent（智能体）需要如下产品路径：Bash 与 PowerShell 子进程（包括经 Bash 运行的钩子命令）默认在受限的文件沙箱下执行；当且仅当沙箱实际拒绝了某个操作时，模型可以为同一操作请求一次用户批准，获批后以更宽的权限重试一次。本设计刻意不声称覆盖所有工具：fs/web/todo 在进程内执行，`execve` 包装层对它们毫无意义（§ 进程内工具）；fs 在自身 seam 强制共享策略，而 web/todo 仍位于文件效果边界之外。如果没有共享词汇，每个工具都会各自重新发明批准字段、拒绝解析、重试匹配和权限状态提示。
...
一个 seam、一条按平台的本地后端链、Bash 与 PowerShell 消费方，加上两个上层控制项：按调用的升级路径与按会话的运行时模式。以下所有内容均从叶子 `cordis.yml` 组合而来；不触及 `agent-loop`。跨工具族 fs 强制、按会话工作区根目录、PowerShell 消费方与 Windows ACL 链使用同一策略载体；剩余阶段——`subagent-acp` 消费方与更多环境——仍列在 § 延迟阶段。
...
这一替换对 `ctx.shell` 的所有消费方透明：Bash 或 PowerShell 工具及其后台任务照常运行，直接使用提供方返回的已包装 argv 启动；钩子命令走 Bash 路径。在上述示例中，删除 `sandbox` 和 `permission` 条目、将 `bash` 替换为 `@deepseek-ai/dsh-bash-local` 即为退出——执行恢复为无约束，升级字段从工具 schema 中消失，因为它们是基于已挂载执行器的能力门控，而非基于配置。仅省略 `approval` 则保留约束但以自身错误文本关闭每次升级；`permission` 还要求 approval seam 和约束执行器同时存在，因此组合不完整的 preset 层会在加载时明确报错。
...
OS 子进程约束适用于 Bash 与 PowerShell 执行器，包括经 Bash 运行的钩子命令，后续还将适用于 ACP subagent 子进程。文件系统、web 和其他工具在进程内执行，需要在各自的 seam 层面实施策略；argv 包装层无法约束一个闭包了 `ctx` 的函数。共享的 shell request/spec 拆分承载按调用的覆盖，而 `tools/pre-execute` 和 approval seam 负责一次性策略决策。
...
`dsh-sandbox-local` 在提供方生命周期内选择一个平台 runner 并缓存结论。Linux 功能性探测 `bwrap` 然后 Landlock；macOS 使用 Seatbelt；Windows 使用 `windows-acl` 受限令牌层级。不支持的平台和不可用的 runner 失败关闭。每次包装携带后端特定的拒绝签名和 runner 失败规则。Landlock runner 失败需要退出码 125，加上一行不是通知的 `landlock-run:` 诊断；即使子进程以 1、2 或 125 退出，精确匹配的部分强制执行通知仍只是信息。Bubblewrap 和 Seatbelt 仍仅依据签名，因为两者的公开约定均未保留 launcher 失败状态；`windows-acl` 保留退出码 127 与一行 `windows-acl-run:` 致命诊断。`runnerCommand` 保留面向运维人员的 `runnerFailureSignatures` 配置，要求其中条目均为非空单行，并将它们映射为一条内部致命规则。每个 shell 消费方都会直接 spawn 返回的 argv，因此缺失的 runner、不可执行的 runner，或 shebang 解释器不可用的可执行脚本会通过可归因的 `ENOENT`／`EACCES` spawn 通道失败，而成功启动的子进程以 126 或 127 退出时仍按普通结果处理。运维人员配置的脚本必然要先启动解释器，再应用自身 profile。
...
`dsh-pwsh-sandbox` 执行相同的提供方 argv 交接，同时保留 `dsh-pwsh-local` 的 PowerShell 命令方言与进程生命周期。

`@deepseek-ai/dsh-shell` 拥有 spawn、runner 失败和拒绝分类器。Bash 与 PowerShell 沙箱提供方消费同一实现，同时各自保留命令方言与进程生命周期代码。拒绝匹配会忽略 trim 后为空的 signature；其他 signature 均保留原值、不经 trim，并以不区分大小写的方式进行子串匹配。对于已结算的后台进程，即使 shell 将信号转换为非零退出状态且保留的 stderr 匹配某个后端拒绝签名，最终的 `status=killed` 仍会阻止拒绝分类。
...
`ctx.sandboxPolicy.resolve()` 在执行器运行前盖章完整执行策略——显式升级模式 > 会话覆盖 > 配置默认值，且 `SessionHeader.cwd` > 配置的后备根目录。每个沙箱化 shell 执行器都在 spec 上保留该策略，或为直接的无 agent 调用方提供部署后备值，使 `run()`/`start()` 永不读取可变会话状态。每进程包装事实以返回的 `ShellProcess` 为键；`onProcessDone()` 会通过 stderr 分类之外的通道接收 spawn 失败，并在 `done` 结算前给该句柄盖章，因此重叠进程各自保留自己的模式和 runner 方言。

当约束执行器被挂载时，对应的 `bash` 或 `pwsh` 工具公布配对的 `sandbox_permissions` 和 `justification` 字段。schema 暴露完整的封闭升级词汇，因为有效模式是按会话的；执行拒绝任何不严格宽于该调用有效模式的目标。批准在执行之前解析。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。

升级是对被拒绝命令的同轮次重试，使用最窄的足够 `sandbox_permissions` 和一个 `justification`；批准提示词是同意步骤。它必须基于实际的拒绝，除非会话已观察到相同的被拒绝访问；禁用或被拒绝的批准终结该命令。重试、批准决策和结果使用既有的工具和批准事件。`dsh-tool-bash` 与 `dsh-tool-pwsh` 分别拥有自己的请求动作，因为执行器 Service Definition 既没有 agent 也没有用户交互所需的 call id。
...
- **Keyless 真实 runner：** 在相应的提供方和 shell 消费方层面对 bwrap、Landlock、Seatbelt 与 Windows ACL 执行真实文件系统效果测试；一个真实 Cordis 上下文通过已交付的 bash 和 fs 工具并发驱动两个项目会话，证明在自身根目录写入成功、在兄弟根目录写入被拒绝。打包安装测试会安装当前 checkout 的原生 tarball，并证明 launcher 保持可执行且字节完全一致。CI 拒绝静默全跳过。
...
- **Subagent 消费方**——`subagent-acp` 可选地约束子 agent（按调用策略；默认无约束——子 agent 必须写入自己的持久化）。
...
- **Windows 后端只提供部分强制执行。** [Windows ACL 沙箱](2026-08-08-windows-acl-restricted-token-sandbox.zh.md)使用受限令牌 runner，并将 Everyone 与硬链接缺口报告为 `enforcement: 'partial'`，绝不声称完整强制。
...
- **在没有后端的平台上会发生什么？** `confine()` 抛出失败关闭的 `SANDBOX_UNAVAILABLE`，命令永不 spawn。Windows 已有 `windows-acl` 受限令牌层级，并将已知缺口报告为 `enforcement: 'partial'`；不支持的平台和不可用的 runner 仍保持失败关闭行为。
...
- **哪些工具实际在约束下运行？** 通过 `ctx.shell` 的 OS 子进程——Bash 与 PowerShell 工具，加上经 Bash 传递运行的钩子命令——以及通过沙箱化 `ctx.fs` 提供方运行的文件系统工具（`read`/`write`/`edit`，见[跨工具族 fs 沙箱 RFC](2026-07-14-cross-family-fs-sandbox.zh.md)）。shell 执行器通过 OS runner 约束，fs 使用进程内路径围栏；二者都以同一个 `ctx.sandboxPolicy` 模式为键。web/todo 仍在进程内且不受限制（web 的唯一效果是网络，不在文件效果模式词汇内）。
...
- **结果上的 `enforcement: 'partial'` 是什么意思？** 所选后端明确报告了一项无法强制所有承诺文件操作的边界。较旧的 Landlock ABI 无法管控路径 truncate 等操作；Windows ACL 后端无法把 Everyone 获准的写入或经外部 NTFS 硬链接别名的写入限制在路径边界内。只有 Landlock 使用探测报告行来区分 `full` 与 `partial`；Windows 因这些已记录的 ACL 限制报告 `partial`。bwrap 和 Seatbelt profile 构造上管控所有承诺的文件操作，因此始终报告 `full`。
...
- `dsh-shell` 的 request/spec 拆分（[shell 词汇目录](../../../../docs/subsystems/shell.zh.md)）——完整的 `sandboxPolicy` 搭载其按调用载体，以及显式 `resolve()` 默认约定。
