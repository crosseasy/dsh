...
- **Windows 后端只提供部分强制执行。** [Windows ACL 沙箱](2026-08-08-windows-acl-restricted-token-sandbox.md)使用受限令牌 runner，并将 Everyone 与硬链接缺口报告为 `enforcement: 'partial'`，绝不声称完整强制。
...
- **哪些工具实际在约束下运行？** 通过 `ctx.shell` 的 OS 子进程——Bash 与 PowerShell 工具，加上经 Bash 传递运行的钩子命令——以及通过沙箱化 `ctx.fs` 提供方运行的文件系统工具（`read`/`write`/`edit`，见[跨工具族 fs 沙箱 RFC](2026-07-14-cross-family-fs-sandbox.md)）。shell 执行器通过 OS runner 约束，fs 使用进程内路径围栏；二者都以同一个 `ctx.sandboxPolicy` 模式为键。web/todo 仍在进程内且不受限制（web 的唯一效果是网络，不在文件效果模式词汇内）。
...
- `dsh-shell` 的 request/spec 拆分（[shell 词汇目录](../../../../docs/subsystems/shell.md)）——完整的 `sandboxPolicy` 搭载其按调用载体，以及显式 `resolve()` 默认约定。
