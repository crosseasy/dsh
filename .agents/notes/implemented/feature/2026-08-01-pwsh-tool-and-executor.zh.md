...
- **`@deepseek-ai/dsh-pwsh-local`** —— `ctx.shell` 执行器 seam 的本地实现，基于 `ctx.subprocess`，逐调用镜像 `dsh-bash-local`：`resolve()` 从配置默认化并设上限，`dsh-shell-runtime` 拥有共享的 `run()` deadline 分类与 `start()` 消费式后台句柄生命周期。命令字符串作为单个 argv 参数传给 `pwsh -NoLogo -NoProfile -NonInteractive -Command`，由 PowerShell 解析，不存在 shell 引号层。可执行文件解析（`resolvePwshPath`）是 `(configured, env, platform)` 的纯函数：先显式配置，再在 Windows 上探测 PowerShell 7 安装位置、PATH 条目（剥离引号）与 Windows PowerShell 5.1，否则返回裸命令名 `pwsh`，交由进程启动时按 PATH 解析。
