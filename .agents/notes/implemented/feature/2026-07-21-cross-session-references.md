...
`@deepseek-ai/dsh-session-reference` is one context consumer service at `ctx.sessionReferenceResolver`. Its outer `agent/pre-step` listener parses canonical mentions in accepted direct user messages and performs the private preparation described by the [public-surface contraction](../simplification/2026-08-25-session-reference-private-preparation.md), without adding reference behavior to a host gateway. The listener derives detached readable content plus an optional identified, frozen `UserMessage` snapshot; core agent packages do not parse session URIs or read another log.
