...
插件不会静态注入 `fs`。因此，不带提供方的产品树仍能正常启动；在文件系统提供方出现之前，插件保持无操作。所有生产读取都通过该提供方完成。候选项探测会解析每个路径并对结果执行 stat，因此会跟随最终路径组件的符号链接至其目标：指向普通文件的链接会被加载，缺失路径或非文件目标则确认为不存在。允许仓库拥有的链接跨越信任边界，是对最初不跟随探测方式的刻意反转；[跟随指令符号链接记录](2026-07-21-follow-instruction-symlinks.zh.md)负责说明该决策及其残余风险。步骤信号与动态工具执行信号会贯穿解析、元数据探测和流式读取，因此取消不会等待无关的文件系统扫描。解析或 stat 异常归类为不可用：它只跳过该候选项，绝不被解释为已经加载的作用域被删除。根标记探测会保留相同的三种状态；仅当系统确认标记不存在时才继续向上遍历，标记不可用则会使初始加载失败。

### 文件名与优先级

默认的逐目录候选列表是 `['AGENTS.md', 'CLAUDE.md']`。该列表可通过 `instructionFileCandidates` 配置；`AGENTS.md` 是普通的第一候选项，而不是隐藏优先级。系统会加载每个现有的普通文件候选项；若同一目录中的多个候选项在去除首尾空白后字节完全一致，则折叠到配置顺序最靠前的候选项。第二个列表 `localInstructionFileCandidates`（默认为 `['AGENTS.local.md', 'CLAUDE.local.md']`）会在基础候选项之后加载叠加式本地覆盖层，并采用相同的逐目录去重；[加载全部候选项决策](2026-07-21-instruction-load-all-dedup.zh.md)与[默认本地覆盖层记录](2026-07-21-local-instruction-overlay.zh.md)分别负责说明这些行为。
...
`maxSourceBytes` 是正数的逐文件上限，默认为 1 MiB。`maxTotalSourceBytes` 是独立的逐批次正数上限，默认值同为 1 MiB，并参与 `baselineIdentity`。基线与协调读取共用同一实现：先规划更具体的目录，在同一目录内保持用于去重的候选配置顺序，并按实际收到的 UTF-8 字节计数。报告的大小无法放入剩余预算时，系统会在读取前拒绝该源文件。对于大小未知或陈旧的情况，提供方可能返回一个超出剩余预算的完整分片；这些字节仍计入预算，该源文件变为不可用，且本批次不再读取更低优先级的源文件。预算耗尽不会产生移除，因此动态协调会保留最近一次有效状态。

loader 通过 `streamText()` 消费内容，并持续统计逐文件及聚合字节数，因此缺失或陈旧的元数据无法迫使其进行无界分配。过大的候选项会被视为不可用，而不是不存在。插件刻意不保留进程级缓存，也绝不保留指令正文。它只为每个有效作用域保存 `{ path, version, digest, trimmedDigest }`，并将这些状态放在 `WeakMap<Session, Map<scope, state>>` 中：提供方 `FsVersion` 与有效提示词状态同时匹配时跳过读取；版本变化则触发有界读取和 SHA-1 确认。SHA-1 仍是持久化在可见结构化元数据中的跨提供方内容标识；提供方版本只作为内存中的失效快速路径。动态缓存转换发生在串行投影组合唯一一条目标 inbox 上下文时；下一次 pre-step 会等待该投影完成，再决定哪些内容进入请求。
...
**每个目录只选择第一个现有候选项。** 不予采纳，因为迁移中的仓库可能在 `AGENTS.md` 和 `CLAUDE.md` 中存放不同指引。加载全部候选项并仅折叠去除首尾空白后内容相同的文件，既能保留两者，也不会重复镜像文件。
...
**把所有根标记错误都当作不存在。** 不予采纳，因为在权限或提供方失败后继续向上遍历，可能跨过真实项目根目录并加载祖先项目的指令。

**把渲染预算同时用作源文件读取预算。** 不予采纳，因为渲染框架和源文件 I/O 的成本不同。独立上限可分别限制提示词大小与提供方读取量。
...
系统由事件驱动，而不是文件监视器驱动。除非文件系统变更通过结构化工具完成，否则编辑不会在确切的文件系统变更时刻可见；外部文件变更会在下一次成功的结构化触碰、恢复对账，或恢复被遮蔽的基线时被发现。这使设计保持确定性并且与提供方无关。源文件 I/O 与渲染输出分别受限；代价是某批次耗尽读取预算时，系统会暂时保留优先级较低的最近一次有效指令。
