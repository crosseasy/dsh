...
A coding agent needs this product path: shell subprocesses — and the hook commands that ride them — execute under a restricted file sandbox by default; if and only if the sandbox actually denies an operation, the model may request one user approval for that same operation and, once granted, retry it once with wider permissions. An every-tool wrapper is deliberately NOT the claim: fs enforces the same mode vocabulary at its own seam, while web/todo execute in-process where an `execve` wrapper is meaningless (§ In-process tools). Without a shared vocabulary, every tool reinvents approval fields, denial parsing, retry matching, and permission-state hints.
...
One seam, one per-platform chain of local backends, shell consumers, and two levers on top: a per-call escalation path and per-session runtime modes. Everything below composes from the leaf `cordis.yml`; nothing touches `agent-loop`. Cross-family fs enforcement, per-session workspace roots, and the Windows ACL backend share the same policy carrier; the remaining phases — the `subagent-acp` consumer, more environments, and additional platform backends — stay under § Deferred phases.

### How a deployment uses it

Five `cordis.yml` entries turn an unconfined coding agent into the sandboxed product path; [`examples/acp-agent`](../../../../examples/acp-agent/README.md) uses this composition by default:
...
The swap is invisible to every consumer of `ctx.shell`: the shell tools, hook commands, and background jobs run by directly spawning the wrapped argv the provider returns. Deleting the `sandbox`, `sandbox-policy`, and `permission` entries and replacing the shell executor with its local provider is the opt-out: execution is unconfined and the escalation fields vanish from the tool schema, because they are capability-gated on the mounted executor, not on configuration. Omitting only `approval` keeps confinement but fails every escalation closed with its own error text; `permission` also requires the approval seam and a confining executor, so a partially composed preset layer fails loud at load.
...
OS subprocess confinement applies to the Bash and PowerShell executors, including hook commands that run through `ctx.shell`, and later to ACP subagent children. Filesystem, web, and other tools execute in-process and require policy at their own seams; an argv wrapper cannot confine a function closing over `ctx`. The shell request/spec split carries per-call overrides, while `tools/pre-execute` and the approval seam own the one-shot policy decision.
...
Left open, for the phase that needs them: whether network restriction arrives as a separate `network_mode` or merges into `sandbox_mode` once a runner enforces both, and whether `SandboxPolicy` accepts extra writable-root grants (the launcher already speaks `--rw <path>`) or only the grants escalation needs.

#### Local backends and the shipped launcher

`dsh-sandbox-local` selects one platform runner per provider lifetime and caches the verdict. Linux functionally probes `bwrap` then Landlock; macOS uses Seatbelt; Windows uses the ACL restricted-token runner. Unsupported platforms and unusable runners fail closed. Each wrap carries backend-specific denial signatures and runner-failure rules so shell-runtime settlement can distinguish a denied file effect from a broken sandbox. Landlock runner failure requires exit 125 plus a non-notice `landlock-run:` line; the exact partial-enforcement line is informational even when a child exits 1, 2, or 125. Windows ACL runner failure requires exit 127 plus a `windows-acl-run:` line. Bubblewrap and Seatbelt remain signature-only because neither public contract reserves a launcher-failure status. `runnerCommand` keeps its operator-facing `runnerFailureSignatures` config, requires non-empty single-line entries, and maps them into one internal fatal rule. The consumer directly spawns every returned argv, so a missing runner, a non-executable runner, or an executable script whose shebang interpreter is unavailable fails through the attributable `ENOENT`/`EACCES` spawn channel while a successfully launched child exit 126 or 127 remains ordinary. An operator-configured script necessarily owns its interpreter startup before it applies its profile.
...
`dsh-bash-sandbox` and `dsh-pwsh-sandbox` wrap their adapter-owned argv through `ctx.sandbox` and directly spawn the provider result. Bash hands over `['bash', '-c', command]`; PowerShell hands over the resolved `pwsh -NoLogo -NoProfile -NonInteractive -Command <command>` invocation. This leaves shell semantics with the inner shell after the runner establishes confinement. A provider error propagates unchanged. A pre-process rejection counts as a runner failure only when the caller-owned workdir is independently usable and Node reports `ENOENT` or `EACCES` with either an `error.path` equal to provider argv[0] or, when `error.path` is absent, an exact `syscall: 'spawn <runner>'`; a present path also requires `syscall: 'spawn'` or the exact `spawn <runner>`. Other codes, invalid workdirs, resource failures, unrelated syscalls, and unstructured rejections retain local command-start semantics. `OneShotSandboxSettlement` converts foreground runner failures to `SANDBOX_UNAVAILABLE` with the original detail; an asynchronous background rejection stamps `runnerFailed: true`, `denied: false`. A `SubprocessRuntime` that synchronously throws the same runner-identifying shape makes background start throw `SANDBOX_UNAVAILABLE`, while other synchronous errors propagate unchanged. After a process starts, foreground and background use one runner-failure classifier that requires the rule's exit-code check and a remaining fatal line after informational exclusions. A match takes priority over denial: foreground execution throws `SANDBOX_UNAVAILABLE` with that fatal line as detail; a settled `ShellProcess` stamps `sandbox.runnerFailed`, and the tool producer renders it through generic `job_output`.
...
`ctx.sandboxPolicy.resolve()` stamps the complete execution policy — explicit escalation mode > session override > configured default, with `SessionHeader.cwd` > configured fallback root — before the executor runs. `SandboxBashExecutor.resolve()` and `SandboxPwshExecutor.resolve()` retain that policy on the spec, or supply the deployment fallback for a direct agentless caller, so `run()`/`start()` never read mutable session state. `OneShotSandboxSettlement` keys per-process wrap facts by the returned `ShellProcess`; `onProcessDone()` passes spawn failure out of band from stderr classification before `done` resolves, so overlapping processes retain their own modes and runner dialects.
...
fs/web/todo execute in-process, so their sandbox semantics are policy at their capability boundaries. The fs seam enforces the shared mode vocabulary through a sandboxed provider (`dsh-fs-sandbox` fences write/edit by mode; see [the cross-family fs sandbox RFC](2026-07-14-cross-family-fs-sandbox.md)), so `read-only`/`workspace-write` are real boundaries for the filesystem tools, not a bash-only approximation. web/todo remain unfenced (web's only effect is network, outside the file-effect mode vocabulary). No generic per-tool sandbox runtime: a host-mediated tool leaves the process only by returning declarative effects the host validates, which is a rewrite, not a wrapper. `ctx.sandboxPolicy` is the shared policy home, with per-capability enforcement instead of a uniform wrapper.

### Testing

- **Unit:** pin platform selection and profiles, direct provider-argv handoff, spawn-level failures with invalid-workdir controls, missing/non-executable/missing-interpreter evidence, malformed-runner negative controls, confined `BASH_ENV` ordering, structured runner classification (including partial-Landlock notice-only child outcomes, gated fatal evidence, child exits 126/127, and foreground/background parity), shell-agnostic settlement, per-call mode/root resolution, per-process facts, escalation validation and outcomes, permission preset folding and write-through, and runtime-context ordering and materialization.
...
- **Additional subprocess consumers** — `subagent-acp` optionally confines child agents (per-call policy; unconfined default — a child agent must write its own persistence).
- **More environments** — an environment-coherent capability group example (e.g. bash+fs against one container).
- **Additional local backends** — a new platform chain needs its own runner, profile dialect, denial signatures, runner-failure rules, and keyless coverage. Wrapping the third-party landstrip runner for Windows was [considered and rejected](../../rejected/feature/2026-07-26-evaluate-landstrip-for-windows-sandbox-rung.md) because it is not battle-tested enough for a security invariant.
...
- **`read-only` is a cross-family boundary.** The [cross-family fs sandbox RFC](2026-07-14-cross-family-fs-sandbox.md) extends the same mode vocabulary to the filesystem tools through a sandboxed `ctx.fs` provider and locates the mode/root config and the `sandbox/mode` override in `ctx.sandboxPolicy` (§ In-process tools).
- **Windows is a partial backend.** The [Windows ACL sandbox decision](2026-08-08-windows-acl-restricted-token-sandbox.md) supplies the restricted-token runner. Its Everyone and hard-link gaps are reported as `enforcement: 'partial'`, never promoted to the full promise.
...
- **The launcher is a workspace dependency in source and an npm dependency after publication.** The main repository tests cover reviewed C source, native CI builds, and byte-pinned local tarballs together; the real-kernel e2e legs vouch for behavior through those installed bytes.
...
- **What happens on a platform with no backend?** `confine()` throws the fail-closed `SANDBOX_UNAVAILABLE` and the command never spawns. Supported local chains are Linux bwrap/Landlock, macOS Seatbelt, and Windows ACL.
