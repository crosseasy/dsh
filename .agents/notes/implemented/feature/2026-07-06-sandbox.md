...
A coding agent needs this product path: Bash and PowerShell subprocesses — including the hook commands that ride Bash — execute under a restricted file sandbox by default; if and only if the sandbox actually denies an operation, the model may request one user approval for that same operation and, once granted, retry it once with wider permissions. An every-tool boundary is deliberately NOT the claim: fs/web/todo execute in-process where an `execve` wrapper is meaningless (§ In-process tools); fs enforces the shared policy at its own seam, while web/todo remain outside the file-effect boundary. Without a shared vocabulary, every tool reinvents approval fields, denial parsing, retry matching, and permission-state hints.
...
One seam, one per-platform chain of local backends, Bash and PowerShell consumers, and two levers on top: a per-call escalation path and per-session runtime modes. Everything below composes from the leaf `cordis.yml`; nothing touches `agent-loop`. Cross-family fs enforcement, per-session workspace roots, the PowerShell consumer, and the Windows ACL chain use the same policy carrier; the remaining phases — the `subagent-acp` consumer and more environments — stay under § Deferred phases.
...
OS subprocess confinement applies to the Bash and PowerShell executors, including hook commands routed through Bash, and later to ACP subagent children. Filesystem, web, and other tools execute in-process and require policy at their own seams; an argv wrapper cannot confine a function closing over `ctx`. The shared shell request/spec split carries per-call overrides, while `tools/pre-execute` and the approval seam own the one-shot policy decision.
...
`dsh-sandbox-local` selects one platform runner per provider lifetime and caches the verdict. Linux functionally probes `bwrap` then Landlock; macOS uses Seatbelt; Windows uses the `windows-acl` restricted-token rung. Unsupported platforms and unusable runners fail closed. Each wrap carries backend-specific denial signatures and runner-failure rules. Landlock runner failure requires exit 125 plus a non-notice `landlock-run:` line; the exact partial-enforcement line is informational even when a child exits 1, 2, or 125. Bubblewrap and Seatbelt remain signature-only because neither public contract reserves a launcher-failure status; `windows-acl` reserves exit 127 plus a `windows-acl-run:` fatal line. `runnerCommand` keeps its operator-facing `runnerFailureSignatures` config, requires non-empty single-line entries, and maps them into one internal fatal rule. Each shell consumer directly spawns every returned argv, so a missing runner, a non-executable runner, or an executable script whose shebang interpreter is unavailable fails through the attributable `ENOENT`/`EACCES` spawn channel while a successfully launched child exit 126 or 127 remains ordinary. An operator-configured script necessarily owns its interpreter startup before it applies its profile.
...
`dsh-pwsh-sandbox` performs the same provider-argv handoff while retaining `dsh-pwsh-local`'s PowerShell command dialect and process lifecycle.

`@deepseek-ai/dsh-shell` owns the spawn, runner-failure, and denial classifiers. The Bash and PowerShell sandbox providers consume the same implementation while retaining their separate command dialect and process-lifecycle code. Denial matching ignores signatures that are empty after trimming; every other signature is matched as its original, untrimmed substring without regard to case. For settled background processes, a final `status=killed` suppresses denial classification even when the shell translates the signal into a nonzero exit status and retained stderr matches a backend denial signature.
...
- **Subagent consumer** — `subagent-acp` optionally confines child agents (per-call policy; unconfined default — a child agent must write its own persistence).
...
- **Windows is a partial backend.** The [Windows ACL sandbox](2026-08-08-windows-acl-restricted-token-sandbox.md) uses a restricted-token runner and reports its Everyone and hard-link gaps as `enforcement: 'partial'`, never the full promise.
...
- **What happens on a platform with no backend?** `confine()` throws the fail-closed `SANDBOX_UNAVAILABLE` and the command never spawns. Windows has the `windows-acl` restricted-token rung and reports its known gaps as `enforcement: 'partial'`; unsupported platforms and unusable runners retain the fail-closed behavior.
...
- **Which tools actually run confined?** OS subprocesses through `ctx.shell` — the Bash and PowerShell tools, plus hook commands transitively through Bash — and the filesystem tools (`read`/`write`/`edit`) through the sandboxed `ctx.fs` provider (the [cross-family fs sandbox RFC](2026-07-14-cross-family-fs-sandbox.md)). Shell executors confine through the OS runner, while fs uses an in-process path fence; both key off the same `ctx.sandboxPolicy` mode. web/todo stay in-process and unfenced (web's only effect is network, outside the file-effect mode vocabulary).
...
- **What does `enforcement: 'partial'` on a result mean?** The selected backend explicitly reports a boundary where it cannot enforce every promised file effect. An older Landlock ABI cannot govern operations such as path truncate; the Windows ACL backend cannot path-confine Everyone-granted writes or writes through an external NTFS hard-link alias. Only Landlock uses the probe's report line to distinguish `full` from `partial`; Windows reports `partial` for those documented ACL limits. The bwrap and Seatbelt profiles govern every promised file effect by construction, so they always report `full`.
...
- The `dsh-shell` request/spec split ([the shell vocabulary catalog](../../../../docs/subsystems/shell.md)) — the complete `sandboxPolicy` rides its per-call carrier, and the explicit-`resolve()` defaulting convention.
