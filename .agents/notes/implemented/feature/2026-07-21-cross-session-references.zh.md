...
`@deepseek-ai/dsh-session-reference` 是注册在 `ctx.sessionReferenceResolver` 上的单一上下文消费服务。它的外层 `agent/pre-step` 监听器会解析已接受直接用户消息中的规范 mention，并执行[公开接口收缩](../simplification/2026-08-25-session-reference-private-preparation.zh.md)所述的私有准备过程，宿主网关无需添加引用行为。该监听器派生分离的可读内容和一份可选的、带标识且冻结的 `UserMessage` 快照；核心 agent 包既不解析会话 URI，也不读取其他日志。
