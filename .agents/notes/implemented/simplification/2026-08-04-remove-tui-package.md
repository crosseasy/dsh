...
DeepSeek Harness ships no first-party terminal UI package. The external `@deepseek-harness-tui/dsh-tui@0.7.1` runtime passes under a pure rc.5 source closure, but public profile assembly remains phase 2 BLOCKED: the historical public-install attempt found 23 missing packages in the direct subset it queried, and a fresh complete query finds exact rc.5 for 0/41 packages in the historical source-validation closure; [Fusion external-plugin ownership](../architecture/2026-08-19-fusion-profile-external-plugin-ownership.md) owns the delivery details and revalidation conditions. This does not restore `packages/ui/tui` or make a TUI installation-owned. Existing imports and `cordis.yml` rows that depend on the deleted package fail instead of being translated. Web remains the installation-owned interactive surface; ACP, JSON-RPC, and one-shot CLI remain the non-Web entry points.
