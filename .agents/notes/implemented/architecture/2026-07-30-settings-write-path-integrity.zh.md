...
在 Service Definition 一侧，`watch()` 的释放器只把观察者从集合中移除——已经接到 watcher 链尾的调用在 dispose 之后照常运行，服务 dispose 时也没有任何环节排空已启动的调用。registration 在有写入进行中时被 dispose 后，旧写入到达存储也可能让 replacement owner 的缓存保持陈旧。`settings/updated` 的手动扇出只捕获同步抛错，异步监听器的 rejection 会以 unhandled rejection 的形式逃逸；`structuredClone` 则放行 Date、Map、BigInt 与循环引用，而 YAML/JSON 存储会在重载往返中悄悄扭曲这些值（Date 会变成时间戳字符串，BigInt 会变成普通数字）。

YAML 写入则整体替换 namespace 节点，把分节内的每条注释都删掉——而这个保注释的提供方承诺过要保住它们。

## 决策

**单一操作链，且每次写入都是读-改-写。**watcher 的刷新与来自各 namespace 队列的持久化共享同一条结算链；`persistSection` 会先把磁盘上的文本对账进 seam——任何未被观察到的差异都先发布出去——然后才对照这份新鲜文本渲染。提供方在其生命周期内按 namespace 持有一个针对**原始**分节的单调 revision；registration replacement 与暂无 owner 的空窗都会保留该值，replacement 本身既不推进它，也不会让针对未变分节的期望值陈旧。基类在排队写入开始执行时检查 `expectedRevision`，并把 `assertRevision` guard 传给 `persist`；每个提供方都在内部对账完成后、首次改变存储前调用该 guard。外部对账或其他**原始**分节变化会让已提供的期望值陈旧，并在存储改变前拒绝写入；省略期望值时仍按后写胜出处理。`publish` 与成功写入都会比较变化前后的**原始**分节，每次变化只将 revision 推进一次。有已注册 owner 时会触发 `settings/document-updated` 并重新解析变化后的分节；无 owner 时只推进 revision，下次 registration 的 descriptor 会暴露累计值。写入不再可能复活陈旧文档，也不会丢掉其他 namespace 或尚未观察到的同级分节；磁盘上已变非法的文档会让写入响亮失败，而不是被覆盖（重载路径保持其「告警并保留最后可用值」策略；共享的 `reconcileFromDisk` 抛错，各调用方自选策略）。watcher 的 `ready` 信号会额外排入一次对账，弥合初始加载与 watcher 生效之间的启动缺口。retired registration 的写入只有在提交点 guard 通过时才会到达存储。若该写入改变分节时已有 replacement owner 拥有对应 namespace，分节被接受时才更新解析值及其 watcher；分节被拒绝时记录告警、保留最后可用的解析值，并且不触发 watcher 或 `settings/updated` 通知。
...
**观察者 dispose 达到完全停稳。**watcher 和 registration 都携带 `active` 标志，排队的调用即将启动时先检查它，因此 watcher 或 registrant 的释放器在调用等待期间运行后能让这次启动彻底不发生。已启动的调用会登记进服务级和 registration 级 `pendingTails` 集合：服务 dispose 在等待各写队列之外等待它们，registrant dispose 则在释放自己仍拥有的 namespace 后等待自己已启动的 watcher work。registration 已 dispose 的 scope 会拒绝后续写入或 watcher。`settings/updated` 扇出会把监听器返回的 thenable 的 rejection 收容进与同步抛错相同的监听器诊断；事件约定现已写明 `INVARIANT` 重抛只服务同步监听器——不变式配套插件必须保持同步，而已交付的那个配套插件本就是同步的。
...
- **把外部编辑合并进正在进行的写入自身的分节**——seam 是在调用时刻可见的状态之上合并 patch 的，因此与写入竞态的同 namespace 外部编辑在调用方省略 `expectedRevision` 时按后写胜出解决；若提供期望值，对账推进该 namespace 的 revision 后会拒绝写入。要把两个值合并起来，需要三方合并语义，而没有任何消费方提出过这种需求。
...
`update()` 对锁获取期限与磁盘文档非法都有成文的失败模式，rejection 消息携带以 `$` 为根的路径。持有者崩溃后可能留下锁，需要操作者核实后移除；若按锁龄自动接管，则会允许多个写入方重叠。仍然存在、且已记录在提供方 README 中的有：同 namespace 并发编辑在调用方不提供 `expectedRevision` 时仍是后写胜出（没有逐值合并）；OS 从未投递的 watcher 事件会让缓存保持陈旧，直到下一个信号或下一次写入；被替换数组内部的注释、以及行内附着在被改标量值上的注释，会随其描述的值一起消失。
