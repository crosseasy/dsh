...
后续的 [Settings wire 快速失败决策](../bug-fix/2026-08-25-fail-closed-settings-wire-description.zh.md)关闭了脱敏前置条件：wire 会拒绝无法证明安全的命名空间、移除 secret 默认值，并在持久化前预检写入。仍然暂缓的是对本次头号能力的组装态测试——用 overlay 挂载一个 fixture 插件（Host 半注册命名空间、`dsh.client` 半注册卡片）并在端到端断言。当前覆盖分别证明了两个半侧；已发卡片输出未变这一点，证明不了新路径。
