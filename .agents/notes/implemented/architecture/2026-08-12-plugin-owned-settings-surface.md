...
The later [fail-closed Settings wire decision](../bug-fix/2026-08-25-fail-closed-settings-wire-description.md) closes the redaction prerequisite: the wire refuses a namespace it cannot prove safe, removes secret defaults, and preflights writes before persistence. [`apps/web/tests/plugin-config-overlay.e2e.ts`](../../../../apps/web/tests/plugin-config-overlay.e2e.ts) boots an overlay fixture package through the real Loader: its Host half registers a namespace with a secret default, its `dsh.client` half registers a keyed card, the HTTP response omits the secret value and default, and the browser renders the card.
