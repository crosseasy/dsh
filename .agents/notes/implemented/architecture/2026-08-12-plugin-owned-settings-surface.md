...
The later [fail-closed Settings wire decision](../bug-fix/2026-08-25-fail-closed-settings-wire-description.md) closes the redaction prerequisite: the wire refuses a namespace it cannot prove safe, removes secret defaults, and preflights writes before persistence. Still deferred: an assembled-composition test of the headline capability — an overlay-mounted fixture plugin whose Host half registers a namespace and whose `dsh.client` half registers a card, asserted end to end. The current coverage proves each half separately; the shipped cards' unchanged output cannot prove the new path.
