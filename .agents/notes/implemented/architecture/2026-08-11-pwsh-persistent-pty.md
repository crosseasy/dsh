...
The `tool-pwsh-persistent` package mirrors `tool-bash-persistent`: same `Config` (`backendType` default `shell`, `timeoutMs`, `maxOutputChars`, `description`) and the same runtime semantics through `@deepseek-ai/dsh-persistent-tool-runtime`. The runtime owns owner-scoped shell caching, first-call de-duplication, per-owner serialization, scrollback paging, deadlines, reset framing, and teardown; the pwsh package owns the PowerShell markers, wrapper, prompt setup, echo stripping, and status parsing. The tool name is `pwsh`; it never co-mounts with the one-shot `tool-pwsh` because the preset rows are mutually exclusive per platform.
