...
- **Allow dependency lifecycle scripts during plugin installation**: rejected because those scripts execute third-party code outside the agent sandbox before bundle admission. Source-only Git packages that require `prepare` must publish a prebuilt npm package or tarball.
