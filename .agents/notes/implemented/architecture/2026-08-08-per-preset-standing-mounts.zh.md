...
- **挂载一旦成功即持续供职，直到组装文件的 stamp 变化。** 运行中会话加入的组装必须在其文件被修改或删除后继续存活；每个代际记录文件 stamp（mtime + 大小），会话发现当前代际已陈旧时，会开启下一个代际，因此文件编辑——创作改为仅复制之后唯一的组装编辑器——无需任何创作调用丢弃指针即可达到后续会话。已加入的会话和冷读取方持有各自使用的代际；被替代的代际只有在替换代际成功挂载后才 retired，并在最后一个 holder 释放时 dispose。[代际回收决策](../bug-fix/2026-08-25-preset-generation-reclamation.zh.md)负责 holder 与 dispose 机制。
