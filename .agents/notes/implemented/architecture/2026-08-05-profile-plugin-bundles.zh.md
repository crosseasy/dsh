...
默认 Profile 模板使用的组合包是 `@deepseek-ai/dsh-base`（共享核心配置行）、`@deepseek-ai/dsh-web-app`（浏览器 Host 配置行与 Web 运行时粘合层）和 `@deepseek-ai/dsh-headless`（直接叠加在 base 上且不含 web-app 的一次性 runner）。通用的 `dsh --profile <name>` 把剩余参数交给该 profile 的命令行启动行：Web 持有自己的 flag 家族，headless 则持有任务位置参数。patch overlay 使用启动器持有的 `--patch`。`dsh plugin --profile <name> <args...>` 负责初始化 profile，以命令配置和环境变量两种方式关闭依赖生命周期脚本后转发给 pnpm，并依据已安装包的组合包声明调和 `dsh.profile.bundles`；没有组合包声明的包保持为普通依赖。[Headless 作为直接 core 入口](2026-08-09-headless-direct-core-entry-point.zh.md)负责 headless 组合约定。
...
- **在插件安装期间允许依赖生命周期脚本**：不采用，因为这些脚本会在组合包准入之前、agent 沙箱之外执行第三方代码。需要 `prepare` 的纯源码 Git 包必须发布预构建 npm 包或 tarball。
