...
- **A settled mount serves until its composition file's stamp changes.** The composition a running session joined must survive its file changing or disappearing; each generation records the file's stamp (mtime + size) and a session that finds it stale starts the next generation, so file edits — the only composition editor once authoring became copy-only — reach later sessions without any authoring call dropping the pointer. Joined sessions and cold readers hold the generation they use; a superseded generation retires only after a replacement mounts successfully, then disposes when its final holder releases. The [generation reclamation decision](../bug-fix/2026-08-25-preset-generation-reclamation.md) owns the holder/disposal mechanics.
