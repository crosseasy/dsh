...
三项相关约束记录在各自适用的位置：投影 key 是否存在不能当作逐会话的能力信号（[`dsh-session-projection`](../../../../packages/session/session-projection/README.zh.md)）；被替代的常驻代际继续向既有 holder 提供服务，并在最后一个 agent 或冷读取方释放后恰好回收一次（[代际回收](../bug-fix/2026-08-25-preset-generation-reclamation.zh.md)）；通过 `cordis_mount` 挂上的临时插件属于组装而非挂载它的会话（[`dsh-tool-cordis`](../../../../packages/extensions/tool-cordis/README.zh.md)）。
