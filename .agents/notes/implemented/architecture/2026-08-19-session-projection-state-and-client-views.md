...
A unit whose key also appears in `SessionProjectionMap` supplies `wire.viewSchema` and `wire.view`. Domain packages may expose their event-to-state folds from Cordis-free modules; production projection definitions and client fixtures call those modules rather than carrying parallel reducers. Every unit's state is checkpointed — client-visible and host-only alike; the `persist` opt-in is gone, so no unit can silently skip the durable cache. Snapshot APIs return only `SessionProjectionMap`, so internal states cannot enter API payloads. Host code reads one current state through `stateOf(session, key)`; the returned reference is borrowed and must not be mutated.
