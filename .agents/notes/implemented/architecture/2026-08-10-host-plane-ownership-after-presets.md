...
Three related constraints are recorded where they apply: projection key presence is not a per-session capability signal ([`dsh-session-projection`](../../../../packages/session/session-projection/README.md)); a superseded standing generation remains available to existing holders and is reclaimed exactly once after its final agent or cold-reader release ([generation reclamation](../bug-fix/2026-08-25-preset-generation-reclamation.md)); and a temporary plugin mounted through `cordis_mount` belongs to the composition rather than the session that mounted it ([`dsh-tool-cordis`](../../../../packages/extensions/tool-cordis/README.md)).
