...
Status: rejected — The inventory mixes completed removals, retained APIs with production consumers, and unimplemented candidates, so it is not a coherent future change.
...
| `CompactionResult.compactionId`, `sourceCommandId`, `startSeq`, `endSeq`, and `summary` | Fixed consumers read `summarySeq` plus shadowed range/seq/token accounting; durable `compaction/*` events own transaction identity, command correlation, start/end seqs, and raw summary content. | Remove those five result echoes while keeping `summarySeq`, the shadowed accounting fields, and both shared transcript renderers. |
