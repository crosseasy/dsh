...
The low-level transport is client-only: it sends requests and accepts correlated responses or notifications. It exposes no notification sender, inbound-request queue, or response helpers. Unexpected server requests and malformed frames are ignored and cannot consume a response waiter; runtime stdout closure fails all outstanding requests.
...
`cwd` and `runtime_cwd` are resolved to absolute paths before subprocess launch, environment injection, and the wire handshake. The public API exposes only applied options: deployment persona and persistence belong in `cordis.yml`, while `session_root` remains the high-level convenience that sets `DSH_SESSION_ROOT`. Transport timeouts and process-closed errors include the retained runtime stderr tail when the runtime has emitted one, even while the stderr reader thread is still alive.
