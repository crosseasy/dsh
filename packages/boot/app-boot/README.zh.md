...
- **后备 identity assertion 只能检测路径竞态**——`healProfilesModuleFallback` 会在每次 `mkdir`、`unlink` 与 symlink 创建前后调用可选的调用方 assertion，但 Node 的路径式文件系统 API 不能让这些操作相对于 descriptor 执行。依赖该 assertion 的调用方必须在 healing 期间独占管理被检查目录。
