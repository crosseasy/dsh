...
| `loadLayeredEnv(binName, cwd?, warn?)` | 构建产品 CLI（命令行界面）冻结的「继承环境 > 项目 `.env` > 用户 `.env`」快照，拒绝文件中的 bootstrap-only 变量，并在不替换继承值的前提下物化其余文件值；内部 smoke guard 会跳过两个文件，并在创建仅含继承环境的快照前被消费 |
...
| `loadOptionalPatches(binName, file, reader?)` | 解析一份可选的 patch 列表文件（即 profile 的 `cordis.patch.yml`）：其顶层是一个 YAML 数组，内容为 include 的 `PatchOptions`（按 id 定位的配置覆盖、`insert` 列表，允许 `!!js`）；可选 `TextFileReader` 提供经调用方校验的字节，文件不存在或 reader 返回 `undefined` 时返回 `undefined`，文件不可读、不可解析或内容不是数组时抛出异常 |
| `loadOverlayPatches(binName, file)` | 解析必需的顶层 YAML 数组，其中包含与上文相同的 include `PatchOptions` 条目；文件缺失也会抛出异常，因为该文件是调用方指名的 |
| `BoundRootConfig` | 调用方持有的 descriptor-bound 根配置文本及 identity assertion；单独提供的配置路径仍是相对解析与诊断锚点且不会被读取，调用方必须保持该 binding 打开，直至使用它的 `mountRootInclude()` 或 `boot()` promise 结算 |
| `mountRootInclude(ctx, absoluteConfigPath, patches?, bareModuleBaseUrl?, root?)` | 注册静态导入的 `cordis:include` 与 `cordis:group` builtin，挂载 include，并保留用户 patch 层 HMR（热模块替换）使用的确切根配置项；可选模块基准会把裸包名锚定到已安装宿主，而相对名称仍以配置目录为基准；可选 `BoundRootConfig` 会按其已记录的调用方持有生命周期替代根文件读取 |
| `watchUserPatches(ctx, options)` | 向现有 Cordis HMR 服务注册指名的 patch 文件；默认在每次新增、变更或移除时读取该文件并将其 patch 传给调用方的 `compose` 闭包，而 `mode: 'compose-read'` 让该闭包负责所有读取；生成的完整 patch 列表以事务方式应用，函数返回异步 disposer |
| `resolveProfileDir` / `initProfile` / `loadProfile` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `healProfilesModuleFallback` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` | Profile 机制（见 [Profile](#profiles)）；`loadProfile` 与 `readProfileManifest` 接受可选 `TextFileReader`，用于读取已经调用方校验的 profile manifest 与 patch 字节，bundle 文件仍按 installation-first 路径读取；`healProfilesModuleFallback` 接受可选 identity assertion，并在每项 fallback mutation 紧邻前后调用 |
| `boot(binName, absoluteConfigPath, patches?, prepare?, bareModuleBaseUrl?, root?)` | 创建根上下文，向 Loader `!!js` 配置表达式暴露 `dshHomePath(...segments)` 并安装 Loader，在配置树条目挂载前执行可选的宿主准备操作（`prepare` 可以使用 Loader，也可以提供由启动器拥有的上下文插槽），再挂载并等待 include 树结算，断言所有条目均已加载并激活，最后返回根上下文——失败时 dispose（资源释放）部分构造的上下文，并以带标签的错误 reject；可选模块基准与 bound root 参数采用 `mountRootInclude` 的相同语义及调用方持有生命周期 |
| `renderConfigDump(binName, absoluteConfigPath, layers, warn?, root?)` | 使用 include 自己的解析器和补丁算法（`entryListSchema`/`applyEntryPatches`）离线合成基础配置与带标签的覆盖层，使结果与 `boot()` 挂载的内容一致，再渲染为 YAML，并原样保留 `!!js` 表达式；可选 `BoundRootConfig` 会替代根文件读取，并且必须保持打开直至同步调用返回；每段来源于同一文件且由相同补丁层修改的连续行之前都有一条 `# ==` 注释，标明该文件和这些补丁层，输出仍是一份可加载的文档；未匹配到行的补丁连同其层标签交给 `warn`（默认：一行 stderr），读取、解析或字段验证失败则抛出 |
...
- **Fallback identity assertion 只能检测路径竞争，不能消除竞争**：`healProfilesModuleFallback` 会在每项 `mkdir`、`unlink` 和 symlink 创建紧邻前后调用可选的调用方 assertion，但 Node 的路径式文件系统 API 无法让这些操作变成 descriptor-relative。依赖该 assertion 的调用方必须在修复期间排他管理受检目录。
