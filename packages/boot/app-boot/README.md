...
- **Fallback identity assertions detect but do not eliminate path races** — `healProfilesModuleFallback` brackets every `mkdir`, `unlink`, and symlink creation with the optional caller assertion, but Node's path-based filesystem API cannot make those operations descriptor-relative. A caller that relies on the assertion must exclusively manage the checked directory while healing.
