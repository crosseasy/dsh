...
function ensureProfileSymlink(link: string, target: string, assertCurrent: () => void = () => {}): void {
  try {
    lstatSync(link)
    return
  } catch (error) {
...
export interface ProfileModuleFallbackOptions {
  ...
  home?: string
  /**
   * Optional caller-owned identity check run immediately before and after
   * fallback mutations.
   */
  assertCurrent?: () => void
}
...
export async function healProfilesModuleFallback(options: ProfileModuleFallbackOptions): Promise<void> {
  const { installAnchor, profile, home = resolveDshHome(), assertCurrent = () => {} } = options
  ...
  if (!moduleFallbackCurrent(modulesDir, entries)) {
    await withFileLock(modulesDir, () => {
      assertCurrent()
      if (!moduleFallbackCurrent(modulesDir, entries)) {
        healProfilesModuleFallbackLocked(entries, modulesDir, assertCurrent)
      }
      assertCurrent()
      return Promise.resolve()
    })
  }
  if (profile !== undefined) healProfileModuleFallback(profile, packageNames, assertCurrent)
}
...
function healProfilesModuleFallbackLocked(
  ...
  for (const entry of entries) {
    ...
    assertCurrent()
    if (entry.kind === 'proxy') {
      ensureModuleProxy(link, entry.packageName, entry.version, entry.targets, assertCurrent)
    } else {
      ensureSymlink(link, entry.packageDir, assertCurrent)
    }
  }
}
...
function healProfileModuleFallback(
  profile: Profile,
  installationPackageNames: ReadonlySet<string>,
  assertCurrent: () => void = () => {},
): void {
  ...
  for (const packageName of ownedPackageNames(ownedModulesDir)) {
    if (!bundleLinks.has(packageName)) {
      removeProfileSymlink(profileModulesDir, ownedModulesDir, packageName, assertCurrent)
    }
  }
  for (const [packageName, target] of bundleLinks) {
    ...
    assertCurrent()
    ensureSymlink(ownedLink, target, assertCurrent)
    const profileLink = join(profileModulesDir, packageName)
    assertCurrent()
    mkdirSync(dirname(profileLink), { recursive: true })
    assertCurrent()
    ensureProfileSymlink(profileLink, ownedLink, assertCurrent)
  }
}
