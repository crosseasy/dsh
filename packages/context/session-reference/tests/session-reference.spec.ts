...
async function listCandidateRows(
  ...
  const candidates = await ctx.sessionReferenceResolver.remoteExportCandidates(agent, query, signal)
...
describe('session reference discovery and preparation', () => {
  it('matches candidate metadata and titles before ranking by cwd', async () => {
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target))).resolves.toEqual([
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target), 'els')).resolves.toEqual([
      { sessionId: SessionId('other'), label: 'other', cwd: '/else', sameWorkspace: false, createdAt: 40 },
    ])
    await expect(listCandidateRows(ctx, fakeAgent(target), 'LATEST')).resolves.toEqual([
      { sessionId: SessionId('same-later'), label: 'Latest title', cwd: '/same', sameWorkspace: true, createdAt: 25 },
    ])
  ...
  it('reads an attached session\'s current title, ahead of any checkpoint', async () => {
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target), 'renamed'))
      .resolves.toEqual([
        { sessionId: live.id, label: 'Renamed mid turn', cwd: '/same', sameWorkspace: true, createdAt: live.header.createdAt },
      ])
    await expect(listCandidateRows(ctx, fakeAgent(target), 'old title')).resolves.toEqual([])
  ...
  it('labels a cold session from its checkpoint and reads no log', async () => {
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target), 'checkpoint'))
      .resolves.toEqual([
  ...
  it('labels a session no projection answers for by its id, still without a log read', async () => {
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target), 'anything')).resolves.toEqual([])
    expect(readTitles).not.toHaveBeenCalled()
    vi.restoreAllMocks()
  })
  ...
  it('still matches an unlabeled session on its own metadata', async () => {
    ...
    await expect(listCandidateRows(ctx, fakeAgent(target), 'source')).resolves.toEqual([
