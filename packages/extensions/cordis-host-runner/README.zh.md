...
- `stop` 回退一次存活的下发——丢弃 handler、把 host 半 fiber dispose（资源释放）到完全停稳、emit `cordis/dynamic-retract`——并让该定义仍然可运行。
...
本功能拥有六条转发事件，由本包在其 client-safe 的 [`./types`](src/types.ts) 子路径上声明，并由 [`@deepseek-ai/dsh-api-remotes`](../../api/remotes/README.zh.md) 的白名单准许投递——正是这一点让浏览器能经 `ctx.remote.$on` 收到它们：`cordis/request-run`（`{requestId, agentId, pluginId, packageId, mode, name, purpose, requiresApproval}`——只有元数据，绝无代码）、`cordis/request-run-resolved`（`{requestId, outcome}`）、`cordis/dynamic-package`（`{pluginId, packageId, pluginRunId, name}`）、`cordis/dynamic-retract`（`{pluginId, packageId, pluginRunId}`）、`cordis/inspect-query`（`{requestId, agentId, provider, method, input}`），以及 `cordis/inspect-query-resolved`（`{requestId}`）。dynamic-package 与 dynamic-retract 是对称的一对运行状态播报：每次全新启动与每次停止都播，与该包有没有浏览器半无关。
