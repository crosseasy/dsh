...
- `stop` unwinds one live dispatch — handlers dropped, host-half fiber disposed to quiescence, `cordis/dynamic-retract` broadcast — and leaves the definition runnable.
...
Six forwarded events belong to this feature, declared by this package on its client-safe [`./types`](src/types.ts) subpath and allowlisted for delivery by [`@deepseek-ai/dsh-api-remotes`](../../api/remotes/README.md), which is what lets a browser reach them through `ctx.remote.$on`: `cordis/request-run` (`{requestId, agentId, pluginId, packageId, mode, name, purpose, requiresApproval}` — metadata, never code), `cordis/request-run-resolved` (`{requestId, outcome}`), `cordis/dynamic-package` (`{pluginId, packageId, pluginRunId, name}`), `cordis/dynamic-retract` (`{pluginId, packageId, pluginRunId}`), `cordis/inspect-query` (`{requestId, agentId, provider, method, input}`), and `cordis/inspect-query-resolved` (`{requestId}`). The dynamic-package and dynamic-retract pair announces run state — every fresh start and every stop, whether or not the package has a browser half.
