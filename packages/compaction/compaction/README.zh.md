...
`CompactionResult` 向调用方返回 summary 事件 seq、已遮蔽范围与 token 计量；持久 `compaction/*` 事件拥有事务身份、命令关联、start/end seq 与原始摘要内容。其结构由漂移检查保障，定义见 [压缩数据结构参考](../../../docs/subsystems/compaction.zh.md#compactionresult)。
