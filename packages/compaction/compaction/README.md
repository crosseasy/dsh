...
`CompactionResult` returns the summary event seq plus the shadowed range and token accounting; durable `compaction/*` events own transaction identity, command correlation, start/end seqs, and raw summary content. Its drift-checked shape lives in the [compaction data-structure reference](../../../docs/subsystems/compaction.md#compactionresult).
