...
Per-preset agent composition. A **preset** is a directory holding one `agent.cordis.yml`; the roster mounts its current generation ONCE per process under a standing scope, and each session that names it joins by having its agent scope key parented to the mount's (`dsh-scope`'s parent chain). The mount's tools, prompt sections, and projection units exist once per active generation and cover every joined agent — its plugins key their state by Session/Agent, so sessions stay apart inside one shared instance — and a host reader with no agent at all (a cold transcript read) leases the same standing registrations by preset id.
...
- `ctx.agentPresets.mount(agentCtx, id?): Promise<AgentPreset>` Compose one agent from a preset — acquire its standing generation (single-flight) and parent the agent's scope key to it until that agent scope unloads — returning the preset for the caller to record. Refuses a broken preset up front with its discovery-reported reason, so every unloadable shape fails the same way before the loader is involved.
...
- `ctx.agentPresets.recompose(agentCtx, id): Promise<AgentPreset>` Re-link one agent to a different preset's standing composition. Valid only while the agent has produced nothing — **the caller owns that check**; the new mount is ensured before the link moves, so a failure leaves the agent as it was. The previous generation holder is released only after the new binding succeeds. Refuses a broken preset like `mount()`.
- `ctx.agentPresets.acquireStanding(id?): Promise<StandingPresetLease>` Acquire the standing generation a host reader with no agent (a cold transcript read) resolves preset registrations in; ensures the mount without starting an agent, session, or turn. The caller must `release()` the lease in a `finally` block so a retired generation can be reclaimed after the read. Refuses a broken preset like `mount()`.
...
`recompose()` ensures the new standing generation, then moves the agent's parent binding and releases the previous generation holder. A failed mount leaves the previous composition linked, and an unknown id is rejected before the binding moves.
