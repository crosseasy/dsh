...
- **`stat`** — returns `FsInfo` for an already resolved target, or `undefined` when absent (`version` is an opaque token derived from bigint `dev:ino:size:mtimeNs:ctimeNs`; `type` is `file`/`directory`/`other`; `size` is in bytes). It checks cancellation before and after the asynchronous metadata probe, so an abort that lands in flight reports `FS_ABORTED` rather than stale absence.
