---
description: "用于精选 profile 的精选插件准入策略目录和只读策略查询服务。"
...
`assertCuratedInstalledLocks(input)` 会同步解析从 descriptor 捕获的根 pnpm lock 与已安装 pnpm lock 字节，选择分配给 `input.profileId` 的 active catalog 候选，并要求其确切 npm 或 Git direct resolution 及完整 runtime 依赖闭包同时匹配 manifest、另一份 lock 和 catalog 摘要。每个 registry 依赖都必须使用 canonical 非占位 SRI，其解码长度必须匹配 SHA-256、SHA-384 或 SHA-512。每个传递 identity 都绑定 peer-suffixed resolution key；direct Git locator 可以携带普通 peer suffix，而 repository、commit 与 package path 校验使用不含该 suffix 的不可变 locator。GitHub codeload identity 还会绑定其 locator 与 resolution 中经过校验的 package path。该函数不访问文件系统、不保留输入字节，并按 catalog 顺序返回带 catalog 目录摘要的深度冻结候选 identity。
