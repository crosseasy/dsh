...
The package exports directory locators for the three asset roots, a read-only `ctx.curatedBench` asset service, and `validateCuratedBenchAssets()` for static asset checks in tests and build gates. Its invariant companion is empty because fixed JSON assets provide no observable event-stream or mutable-data relationship. Consumers read explicit JSON files from those directories; the package does not run benchmarks itself.
