---
description: "把受治理 profile 写入 DSH home 的精选 profile 模板与物化 helper。"
...
- `materializeCuratedProfile(profileName, home, options)` 把所选 profile 写到 `home` 下，并返回 profile 目录；`options.userLayer: false` 会在物化期间跳过已有用户 patch 解析，但精选 `--dump-default-config` 仍会解析并准入 profile patch，再从输出中省略该层。
- `materializeCuratedProfileForLoad(profileName, home, options)` 物化 profile，并返回绑定到 descriptor 的受管文件字节；调用方必须保持该快照打开，直至使用它的 `boot()` 或 `mountRootInclude()` promise 结算。
...
- `assertCuratedProfileLockAdmission(profileName, locks)` 加载签入 catalog，派生内置 profile 的确切生成依赖，并通过 curated policy 准入从 descriptor 捕获的根 pnpm lock 与已安装 pnpm lock 字节。
...
物化会在发布缺失文件前校验全部已有受管输入，把每个新文件纳入同一 descriptor snapshot，只在并发发布文件的字节一致时接纳它，并在后续发布或 identity 检查失败时按逆序删除本次调用创建的文件。非法 profile patch YAML 的 parser failure 会用脱敏占位符替换源 identity、源码内容与 parser 原因，同时保留从 1 开始的行列号。非法 workspace 与 dependency allowlist YAML 诊断仍使用 policy redactor，并丢弃 parser exception。Enterprise patch 拒绝会指出被禁止的 override 类别与修正操作，且不会回显该值。物化与准入在添加所选依赖前使用 curated-policy 的当前 profile 完整性谓词；聚焦单元测试会校验模板与依赖的一致性，仓库 `verify-curated-activation-evidence` 门禁则检查 catalog 与模板之间的关系。包 invariant 是带说明的空 installer，因为固定模板不提供可观测的运行时关系。
...
- **启动与安装准入不可跳过**：`dsh` 启动和配置 dump 要求 bundle 顺序与模板及 catalog 分配精确一致，保持 installation-first 解析，并拒绝 profile、home 与命令行 patch 中的动态表达式或未批准 plugin/group 插入。精选安装还会对照确切 catalog 来源 identity 与 runtime closure 摘要准入从 descriptor 捕获的两份 lockfile，计算每个 selected installed candidate 的目录摘要，并在激活 rename 后复核完整目录。每次静态组合和用户 patch 热重载后都会再次执行 enterprise 限制。`--dump-default-config` 恢复诊断会解析 profile patch 并执行准入，但不在输出中包含该层；它不会读取 home 或命令行 patch 层。独立 observed preflight 保留其单独的产物目录校验。
