---
description: "Curated profile templates and materialization helpers for writing governed profiles into a DSH home."
...
- `materializeCuratedProfile(profileName, home, options)` writes the selected profile under `home` and returns the profile directory; `options.userLayer: false` skips existing user-patch parsing during materialization, while curated `--dump-default-config` still parses and admits the profile patch before omitting it from output.
- `materializeCuratedProfileForLoad(profileName, home, options)` materializes the profile and returns descriptor-bound managed-file bytes that the caller must keep open until the consuming `boot()` or `mountRootInclude()` promise settles.
...
- `assertCuratedProfileLockAdmission(profileName, locks)` loads the checked-in catalog, derives the exact generated dependencies for the built-in profile, and admits descriptor-captured root and installed pnpm lock bytes through curated policy.
...
Materialization validates all existing managed inputs before publishing missing files, retains each new file in the same descriptor snapshot, accepts a concurrently published file only when its bytes match, and removes files created by the current call in reverse order if a later publication or identity check fails. Malformed profile-patch YAML parser failures replace the source identity, source content, and parser reason with redacted placeholders while retaining one-based line and column. Malformed workspace and dependency-allowlist YAML diagnostics use the policy redactor and discard the parser exception. Enterprise patch rejections name the prohibited override category and corrective action without echoing its value. Materialization and admission use curated-policy's current-profile completeness predicate before adding a selected dependency; focused unit tests validate template and dependency consistency, while the repository `verify-curated-activation-evidence` gate checks catalog-to-template relationships. The package invariant is an explained empty installer because fixed templates expose no runtime relationship to observe.

<a id="scenario-policy"></a>
## Scenario policy
...
- **Boot and install admission are mandatory**: `dsh` startup and config dump require exact template-order bundles and catalog assignments, retain installation-first bundle resolution, and reject dynamic expressions or unapproved plugin/group insertions from profile, home, and command-line patches. Curated installation additionally admits both descriptor-captured lockfiles against exact catalog source identities and runtime closure digests, hashes each selected installed candidate tree, and rechecks the complete tree after activation rename. Enterprise restrictions apply again after every static composition and live user-patch reload. The `--dump-default-config` recovery diagnostic parses and admits the profile patch but omits it from output; it does not read home or command-line patch layers. The separate observed preflight retains its independent artifact-tree verification.
