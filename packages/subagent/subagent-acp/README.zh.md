...
`dispose()` 使用 subagent seam 共享的进程外运行句柄，因此重复调用会返回同一个记忆化清理 promise。清理会移除信号监听器，在可行时请求 ACP 取消，然后使用 subprocess seam 定义的操作运行本后端自有的拆卸阶梯（`disposeAcpChild`）：先关闭 stdin 并等待 `disposeEofGraceMs` 让子进程协作式完全停稳，再触发句柄的 `terminate()` 升级（SIGTERM、spawn 宽限期、SIGKILL——Windows 直接强制终止），并等待子进程责任方给出整棵进程树的退出证明。每次运行都使用全新进程；尚未实现进程池。
