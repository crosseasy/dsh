...
function nodeOptionsWithDisabledExperimentalWarning(value: string | undefined): string {
  const options = value?.trim().length === 0 || value === undefined ? [] : value.split(/\s+/u)
  return options.includes(DISABLE_EXPERIMENTAL_WARNING_OPTION)
    ? options.join(' ')
    : [...options, DISABLE_EXPERIMENTAL_WARNING_OPTION].join(' ')
}
