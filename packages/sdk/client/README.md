...
- **No server→client requests** — `HarnessClient` receives only responses and notifications. The shared low-level client transport retains outbound `notify()` for the Codex app-server's required `initialized` handshake, but this SDK client does not expose notifications.
