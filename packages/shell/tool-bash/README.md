...
The package root exposes only the Cordis plugin contract (`name`, `inject`, `Config`, `apply`). This package still owns tool registration, schema text, approval and escalation routing, workdir/default policy, prompt text, tool identity, job kind, and the bash dialect contract. Foreground projection, foreground/background result rendering, background-read rendering, background output schema properties, and background process outcome mapping reuse pure helpers from `@deepseek-ai/dsh-shell`.
...
Result text contains stdout, an optional `[stderr]` section, then applicable sandbox-denial, timeout, signal, exit-code, and truncation markers. The shared `@deepseek-ai/dsh-shell` renderer owns this marker order and spill wording. Timeout is reported independently of final exit status; nonzero exit remains a model-interpreted result rather than `isError`. Truncation links a safe complete spill file or reports it unavailable. Only infrastructure failures such as spawn errors and aborts produce `isError`.
