...
| [`pwsh-sandbox/`](pwsh-sandbox/README.md) | Applies the configured [`sandbox`](../sandbox/README.md) backend to PowerShell execution, including the Windows `windows-acl` chain. | (registers `ctx.shell`) |
...
| [`tool-bash-persistent/`](tool-bash-persistent/README.md) | Exposes persistent Bash sessions backed by the terminal service to the model. | (consumes `ctx.terminals`; registers on `ctx.tools`) |
| [`tool-pwsh/`](tool-pwsh/README.md) | Exposes PowerShell execution and background-job integration to the model. | (registers on `ctx.tools`) |
