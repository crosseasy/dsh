...
The package root exports the default and named `SandboxBashExecutor` plugin plus its `Config`; shared settlement and result-classification helpers live in `@deepseek-ai/dsh-shell-runtime`.
...
- Process mechanics (spawn, process-group kills, output collection/spill, background handles, credential scrub) and sandbox settlement use [`dsh-shell-runtime`](../shell-runtime/); bash argv/environment policy stays in [`dsh-bash-local`](../bash-local/), and runner selection lives in [`dsh-sandbox-local`](../../sandbox/sandbox-local/).
