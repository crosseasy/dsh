...
This package owns the Service Definition role of the shell capability shared by Bash and PowerShell providers and consumers. The roles are split so each can evolve (and be swapped) independently:

| Package | Role |
|---|---|
| `@deepseek-ai/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@deepseek-ai/dsh-bash-local` / `@deepseek-ai/dsh-pwsh-local` | Service Providers: local Bash / PowerShell subprocesses |
| `@deepseek-ai/dsh-bash-sandbox` / `@deepseek-ai/dsh-pwsh-sandbox` | Service Providers: the corresponding local executor's mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@deepseek-ai/dsh-tool-bash` / `@deepseek-ai/dsh-tool-pwsh` | the model-facing tool schemas over `ctx.shell` |

The split is a standard capability seam ([capability-seams Agent Note](../../../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)): the Bash and PowerShell sandbox providers are sandboxing executors behind the same Service Definition — each Consumer detects its executor's `sandboxMode` capability and adds escalation fields without importing the provider — and a containerized or remote executor slots in the same way.
...
`stdin` and ordinary `env` are set by in-process plugins (the hooks bridges, native plugins) to feed a hook command its JSON payload and `CLAUDE_PROJECT_DIR`/`CLAUDE_PLUGIN_ROOT` values. `dshEnv` is a separate trusted overlay restricted by type to managed keys; the exported `DSH_ENV_PREFIX` is the single source for that namespace, its `DshEnvironmentKey` template type, executor scrubbing, registry validation, derived built-in names, and model guidance. The model-facing Bash and PowerShell tools use the current snapshot collected by `ctx.shellEnv`. Implementations remove inherited managed keys, then merge `dshEnv` after ordinary `env`, so an omitted current fact cannot fall back to stale ambient state and an `env` entry cannot displace a managed value. Neither model-facing tool exposes these inputs as parameters. All three remain optional on the resolved spec; absent means no input/overlay. See [the bash-stdin-env Agent Note](../../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md) and [the session environment Agent Note](../../../.agents/notes/implemented/feature/2026-07-10-agent-session-identity-and-log-location.md).
...
The package root also exports `isSandboxRunnerSpawnFailure`, `classifySandboxRunnerFailure`, `classifySandboxDenial`, and `matchesSandboxSignature`. Bash and PowerShell sandbox providers use this shared classification for spawn provenance, structured runner failures, and denial signatures.
