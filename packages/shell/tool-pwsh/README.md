...
The package root exposes only the Cordis plugin contract (`name`, `inject`, `Config`, `apply`). This package still owns tool registration, schema text, approval and escalation routing, workdir/default policy, prompt text, tool identity, job kind, and the PowerShell dialect contract. Foreground projection, foreground/background result rendering, background-read rendering, background output schema properties, and background process outcome mapping reuse pure helpers from `@deepseek-ai/dsh-shell`; the package-local render/background modules keep the pwsh export names as thin aliases.
