...
Local PowerShell Service Provider for the `@deepseek-ai/dsh-shell` executor seam over the [`@deepseek-ai/dsh-subprocess`](../../subprocess/subprocess/README.md) service: `PwshLocalExecutor` spawns `pwsh -NoLogo -NoProfile -NonInteractive -Command <command>` per call as a managed process through `ctx.subprocess`, owns PowerShell executable resolution, request resolution, and environment policy, and delegates shared one-shot lifecycle to [`dsh-shell-runtime`](../shell-runtime/README.md). Group mechanics (bounded spill-backed output, credential scrub, kill escalation, disposal) are the subprocess service's.
