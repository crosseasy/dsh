...
The caller's agent is the parent of every fresh child, preserving cwd and lineage without copying its conversation. An already-aborted `exec.signal` prevents startup; after `start()` returns, later `exec.signal` aborts are bridged to `run.cancel()`. The tool awaits `run.result` and calls `run.dispose()` in `finally`, so a cancelled parent step waits for the engine's bounded termination and child quiescence before returning.
