...
调用方 agent 是每个全新子 agent 的父级，因此会保留 cwd 和谱系，但不会复制其对话。已经中止的 `exec.signal` 会阻止启动；`start()` 返回后，工具会添加一条最多调用一次 `run.cancel()` 的桥接，并立即复查同步启动期间是否发生中止。工具在 `finally` 中调用 `run.dispose()` 前移除该桥接，因此取消的父级步骤会等到引擎完成有界终止且子 agent 完全停稳后才返回。
