/** Private runtime installer shared by the production and testing entry points. */
...
function validateConfig(config: Readonly<Record<string, never>>): void {
 ...
 * @param internals - optional deterministic hooks for repository tests.
 */
export function installRetryPolicy(
  ...
  async function recover(
    ...
    const priorPolicyRetry = retryState[retryStateKey(provider, policyKey)]
    const previousRetry = priorPolicyRetry?.retry ?? 0
    if (policy.mode === 'normal' && previousRetry >= policy.maxRetries) return next()
    const retry = previousRetry + 1
    const retryId = priorPolicyRetry?.retryId ?? RetryId(randomUUID())
    let delayMs: number
