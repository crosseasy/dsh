...
All three units use the standard projection baseline, live frame, higher-seq-wins store, and JSON checkpoint paths. Their Cordis-free `init`, `apply`, and `view` functions and complete-log helpers are exported through `./client`; the production projection definitions and the development fixture both call them rather than maintaining parallel client-only reducers. Unloading token-meter removes all three keys. A composition without the projection seam keeps the measurement service's existing behavior.
