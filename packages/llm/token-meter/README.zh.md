...
三个单元都使用标准的投影基线、实时帧、seq 高者胜值仓和 JSON 检查点路径。它们的事件 fold 位于不依赖 Cordis 的模块中，并通过 `./client` 导出；生产投影定义与开发 fixture 都调用这些 fold，而不是维护一套平行的客户端 reducer。卸载 token-meter 会移除这三个键。不带投影 seam 的组合会保留测量服务的既有行为。
