...
三个单元都使用标准的投影基线、实时帧、seq 高者胜值仓和 JSON 检查点路径。它们不依赖 Cordis 的 `init`、`apply`、`view` 函数与全日志 helper 都通过 `./client` 导出；生产投影定义与开发 fixture（测试前置数据）都调用这些函数，而不是维护一套平行的客户端 reducer。卸载 token-meter 会移除这三个键。不带投影 seam 的组合会保留测量服务的既有行为。
