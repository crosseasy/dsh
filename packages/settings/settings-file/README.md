...
- **Every write is a read-modify-write.** A persist first re-reads the document and publishes any difference into the seam — an external edit still inside the watcher debounce window, a change the watcher missed, or another process's write — then checks `expectedRevision` before rendering or changing storage. A stale expectation rejects without overwriting the reconciled same-namespace value; other namespaces and unobserved sibling sections remain intact. If the on-disk document turned invalid, the write rejects loud instead of overwriting the user's manual edit.
...
- **Same-namespace writes without `expectedRevision` stay last-write-wins** — the writer lock and read-modify-write keep concurrent writers from dropping each other's namespaces, but unguarded writes to one namespace still resolve to the later write. `expectedRevision` adds whole-section conflict rejection, not per-value merge.
