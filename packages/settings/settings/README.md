...
- `register(ns, schema, { base?, applies? })` — returns the owner `SettingsScope` (`get`/`watch`/`update`). The registration is an effect on the calling plugin's fiber: disposing that fiber marks the scope inactive, removes the namespace and its observers, skips watcher invocations that have not started, and waits for started watcher invocations before resolving. A disposed scope rejects later writes or watchers; a stored section the schema rejects fails the registration itself; a duplicate namespace fails loud.
- `describe()` — one verbatim descriptor per namespace (`schema.toJSON()` envelope, resolved value, detached `base`/`user` layers, `applies`) for same-process consumers; a field's presence in `user` is what marks it user-overridden.
- `describeForWire(ns?)` — the only supported wire description path. It proves the complete schema graph safe before serialization, strips `role('secret')` fields from every value layer and schema default, and adds the `secrets` slot list (`{ path, set }`). A secret under a union or intersection, any transform, or an unsupported schema node rejects the namespace with a value-free error; selecting `ns` supports a write preflight before persistence.
...
- Service teardown refuses new writes and watcher starts, then drains every queued write and every started watcher invocation before disposal completes. A write whose registrant fiber was disposed mid-flight still reaches storage; if another registration owns the namespace when it lands, that owner re-resolves the committed raw section, advances its revision, and notifies under its own watcher rules. If the replacement owner rejects that section, the publish path's last-good warning behavior applies and the replacement cache stays unchanged.
...
- **Wire schemas use a deliberate subset of Schemastery** — direct secret leaves under `object`/`dict`/`array` are supported. Transforms, extension-defined nodes, and secrets under unions or intersections are rejected because their callbacks or branch-dependent paths cannot be represented safely. Plugins that need those constructs keep them outside a wire-exposed settings namespace.
