...
export abstract class SettingsProvider extends Service {
  ...
  register<const Namespace extends string, T>(
    ...
    if (!this.revisions.has(parsedNs)) this.revisions.set(parsedNs, 0)
    ...
    this.ctx.effect(() => {
      ...
      return async () => {
        ...
        if (this.registrations.get(parsedNs) === registration) {
  ...
  private descriptorLayers(registration: SettingsRegistration): Omit<WireDescriptionInput, 'value'> {
    ...
    try {
      ...
      // last good resolved value; describing it as no user layer keeps this read total.
      user = undefined
    }
  ...
  async update<const Namespace extends string>(
    ...
    const parsedNs = parseSettingsNamespace(ns)
    const registration = this.registrations.get(parsedNs)
    if (registration === undefined) {
      throw new Error(`settings namespace "${parsedNs}" is not registered`)
    }
