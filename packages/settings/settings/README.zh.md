...
- `describeForWire(ns?)` — 唯一受支持的 wire 描述路径。它在序列化前证明完整 schema 图安全，从每个值层和 schema 默认值中剥离 `role('secret')` 字段，并附加 `secrets` slot 列表（`{ path, set }`）。字段缺失或 secret 字典为空时，`set` 为 false；其他已存在的值均为 true。union 或 intersection 下的 secret、任何 transform 或不受支持的 schema 节点会让 namespace 以不含值的错误被拒绝；选择 `ns` 可在持久化前完成写入预检。
...
- 每次写入都可携带可选的 `expectedRevision`。提供方在其生命周期内按 namespace 持有 `revision`，descriptor 暴露其当前值。该计数器只在**原始**分节变化时推进，并跨 registration replacement 与暂无 owner 的空窗保持单调；replacement 本身既不推进计数器，也不会让针对未变分节的 `expectedRevision` 陈旧。基类在排队写入开始执行时检查期望值，提供方在其内部对账完成后、改变存储前再次检查。期望值不匹配时，写入会以 `SettingsConflictError`（`code: 'SETTINGS_CONFLICT'`，并附上两个 revision）被拒绝；省略期望值时仍按后写胜出处理。
...
- 服务卸载先拒绝新写入与观察者调用的启动，再排干全部排队写入与已启动的观察者调用后才完成。registrant fiber 在写入途中被 dispose 后，写入只有在提供方的提交点 revision guard 通过时才会改变存储。registration replacement 本身不会让 guard 失败；外部对账或其他**原始**分节变化会让已提供的期望 revision 陈旧，并在存储改变前以 `SettingsConflictError` 拒绝写入，省略 `expectedRevision` 时则仍按后写胜出处理。旧写若改变**原始**分节，提供方会将 revision 推进一次；存在当前 owner 时，该 owner 会重新解析分节、收到一次 document event，并按自己的 watcher 规则通知，而无 owner 时只静默推进，供下次 registration 的 descriptor 暴露。若 replacement owner 拒绝该分节，则沿用 publish 路径的 last-good 告警行为，并保持 replacement cache 不变。

## 提供方约定

子类实现 `writable`、`load()`、`persist(ns, section, assertRevision)`，可选择为一个本地用户可编辑文件重写 `documentPath` 与 `prepareDocument()`，并通过受保护的 `publish(doc)` 推入外部观察到的文档。提供方必须在内部对账完成后、首次改变存储前调用 `assertRevision()`。基类服务 init 在服务可注入前加载并发布一次文档；拥有自有 init（watcher、连接）的提供方会先通过 `yield* super[Service.init]()` 委托给基类。publish 时每个已注册 namespace 独立重解析：非法分节保留该 namespace 的最后可用值并告警——热重载绝不拖垮进程；启动期与注册期校验则立即报错。

## 事件

`settings/updated (ns, next, prev, source)` 在每次提交后触发；`source` 为 `update`（进程内写入）或 `provider`（外部变更）。解析值深相等时绝不触发——它面向消费方，而消费方只关心自己的值有没有变。

`settings/document-updated (ns, revision)` 在 namespace 有已注册 owner 且**原始**用户分节发生变化时触发，无论解析值是否随之改变。配置界面需要的是这一个：存入一个与组合 `base` 相同的覆盖值不会改变解析值，却改变了文档的说法（该字段从继承变成了覆盖），也推进了每个已打开编辑器所持有的 revision。无 owner 时的**原始**分节变化只静默推进 revision，下次 registration 的 descriptor 会暴露累计值；registration replacement 本身既不推进 revision，也不触发事件。监听器的异常隔离方式与 `settings/updated` 相同。
...
- **跨进程并发由提供方定义** — seam 仅在进程内按 namespace 串行化写入；跨进程并发按提供方行为收敛。本地文件提供方在写锁下读-改-写，保留其他 namespace 与尚未观察到的同级分节；`expectedRevision` 会在对账后拒绝陈旧的同 namespace 写入，省略该参数的写入仍按后写胜出处理。
