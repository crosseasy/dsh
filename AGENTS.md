...
- Publishable dsh packages set `publishConfig.access: public`; experimental/private packages remain private. Vendored packages are rescoped ([mapping](docs/rescope.md)) and follow their publication policy. `@deepseek-ai/cordis` is a peerDependency (+ dev) of every harness package.
