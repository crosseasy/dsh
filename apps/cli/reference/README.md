...
For a built-in curated profile, normal startup and `--dump-config` run mandatory boot admission before Loader activation. The manifest bundle list and dependencies must match the template and catalog assignment, package-manager settings must remain safe, and profile, home, and command-line patch layers may only apply static overrides to approved rows; dynamic expressions and plugin/group insertions reject. Enterprise restrictions apply to the final composition and every live user-patch reload. `--dump-default-config` remains the bundle-only recovery diagnostic: it validates the manifest and package-manager state without parsing the profile, home, or command-line user patches. Observed curated preflight separately verifies installed package trees and both pnpm lockfiles.
