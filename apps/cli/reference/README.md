...
For a built-in curated profile, normal startup and `--dump-config` run mandatory boot admission before Loader activation. The manifest bundle list and dependencies must match the template and catalog assignment, package-manager settings must remain safe, and profile, home, and command-line patch layers may only apply static overrides to approved rows; dynamic expressions and plugin/group insertions reject. Enterprise restrictions apply to the final composition and every live user-patch reload. `--dump-default-config` remains the bundle-only recovery diagnostic: it parses and admits the profile patch but omits that layer from the rendered output, and it does not read the home or command-line patch layers. Observed curated preflight separately verifies installed package trees and both pnpm lockfiles.

## Plugin management

`dsh plugin --profile <name> <args...>` initializes the profile when missing (shipped template, or `@deepseek-ai/dsh-base` alone for other names), then forwards `<args...>` to `pnpm` with the profile directory as working directory and dependency lifecycle scripts disabled; pnpm must be on PATH. Relative path specs (`.`, `../plugin`, and their `file:`/`link:` forms) are anchored to the invoking directory first, so `add .` from a plugin checkout installs that checkout, not the profile. After every successful run, `dsh.profile.bundles` is reconciled against the installed state: each dependency resolving to a package whose manifest declares `"dsh": { "bundle": { "patch": "./cordis.patch.yml" } }` joins the layer stack (so an `update` that gains the declaration activates it), a bundle-less dependency stays plain with a one-time warning, and a removed dependency leaves the stack.
...
Plugin management never runs dependency lifecycle scripts. Git-hosted source packages that require `prepare` are therefore unsupported; publish a prebuilt npm package or tarball instead. Passing an `ignore-scripts` override is rejected. Built-in curated profiles have fixed bundle membership. A bare `dsh plugin --profile <curated-name> install` holds a cross-process writer lock, reclaims prior interrupted staging and activation state, creates a private staging home, and installs exactly the template dependencies from the local pnpm store. It preserves a validated existing profile patch and lockfile, adds `--frozen-lockfile` when that lockfile exists, removes ambient npm/pnpm configuration, pins pnpm's user and global config to the staged `.npmrc`, rejects package-manager root redirects, transformations, and build-grant environment overrides, and accepts no extra arguments. The staged profile must pass generated-file, root and installed lockfile, bundle-resolution, and curated-admission checks before its directory is activated. Failures before activation leave the previous live directory unchanged. After activation starts, rollback restores the previous directory only while the activated and previous directory identities remain unchanged; an identity change can make rollback fail, and rollback does not overwrite the replacement. A missing local package fails rather than fetching from the network. `--help` and `list` are generated from checked-in template data without creating profile state or invoking pnpm; `add`, `remove`, every other package-state command, package transformations, and dependency build grants reject before pnpm runs. Other profiles retain the general pnpm forwarding behavior.
