...
[本地提供方](../../packages/sandbox/sandbox-local/README.md) 拥有运维配置，并将其 runner 方言映射到这些规则。`@deepseek-ai/dsh-shell` 拥有供 [Bash](../../packages/shell/bash-sandbox/README.md) 与 [PowerShell](../../packages/shell/pwsh-sandbox/README.md) 沙箱提供方共用的 spawn 来源归因、runner 失败和拒绝分类；两个提供方各自保留命令方言与进程生命周期。
