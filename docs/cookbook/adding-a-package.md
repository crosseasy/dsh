...
package.json invariants (enforced by `pnpm run constraints` / `scripts/check-workspace-constraints.ts`): every non-experimental DSH package is a release member, omits `private: true`, declares `publishConfig.access: "public"` and its exact repository directory, and carries a `version` matching the root `package.json`; packages under `packages/experimental/` instead remain private and omit `publishConfig`. Every package uses `type: module`, `main: "lib/index.js"`, `types: "lib/types/index.d.ts"`, exports both `.` and `./invariant` from built files, and declares `@deepseek-ai/cordis` in BOTH peerDependencies and devDependencies (same range). Mirror every dsh peer dependency in devDependencies. `@deepseek-ai/dsh-invariants` is always a `workspace:^` devDependency; `pnpm run verify-package-invariants` additionally requires it as a peer unless the repository's package-dependency policy flattens that package's non-Cordis peers. The package `tsconfig.json` references `runtime-diagnostics/invariants`. `@deepseek-ai/schemastery` goes in `dependencies` (it is a runtime validator), matching agent-loop. The `files` list contains exactly `lib/index.js`, `lib/invariant.js`, `lib/types/**/*.d.ts`, and package-specific runtime artifacts recognized by the gate; a package whose runtime export points into the emitted tree also includes `lib/types/**/*.js`. Do not publish `src`, declaration maps, JS maps, or stale root declaration files. CLI app packages with a package `bin` include `lib/bin.js` immediately after `lib/index.js` in `files`.
