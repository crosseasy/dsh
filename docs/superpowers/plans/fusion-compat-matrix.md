...
Checked through: `2026-08-22T17:01:07Z`
...
Current result: the Fusion Web external set contains exact Pet and Git Graph `0.2.9`. Task 22 covers the consecutive `0.2.8` and `0.2.9` waves for all 17 identities; all 34 exact tarballs pass identity and integrity. Task 26 rejects ModLens `3.24.0` at server-side request safety. Task 28 audits every Better Sidebar release after the prior cutoff and rejects the sole new candidate, exact `0.15.2`, at the public rc.5 peer closure; security, lifecycle, isolated installation, composition, boot, and Chrome are `NOT RUN`. These blockers do not change the selected rows. Historical zero-row, three-row, four-row, and six-row results remain superseded evidence. TUI `0.7.1` source runtime passes, public delivery remains phase 2 BLOCKED, and Liangshen remains sourced from `0.2.4`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| final two-row Web composition | Pet and Git Graph `0.2.9` | PASS: exact identity, integrity, and license | PASS: exact packages plus React `18.3.1`; no native build approval | PASS: exact `pet` and `ui-git-graph` rows | PASS: isolated baseline/Pet/Git runs | PASS: combined gate shows one Pet root/dock, one Git chip, live Pet-state and Git-branches probes, clean CDP diagnostics | **PASS** |
...
| `@linxin666/dsh-client-ui-git-graph` | accepted `0.2.9` | PASS exact license and authorization | PASS exact isolated install | PASS: one `ui-git-graph` row | PASS | PASS: live Git status, one branch chip, clean diagnostics | **PASS** |
...
Pet and Git Graph `0.2.9` are accepted; the other six decision-bearing external Web capabilities remain blocked. ModLens `3.24.0` and Better Sidebar `0.15.2` stop before candidate Chrome validation, so their Chrome results are `NOT RUN`; the current two-row acceptance remains the latest system Chrome CDP `9333` result. TUI runtime passes under the pure rc.5 source closure, but public delivery remains a phase 2 blocker until a consistent rc.5 closure is publicly available or a new Harness baseline is explicitly approved and fully revalidated.
...
Pet and Git Graph `0.2.9` each pass unpaired, revoked, paired, and loopback authorization probes. Pet's exact client lifecycle tests pass 5/5. Git Graph's direct Host probe disposes both routes and an open SSE stream, remounts on the same registry without duplication, and its independent RED run detects retained routes, an open SSE response, active timers, and duplicate remount before forced cleanup returns all counts to zero. Its exact client apply/dispose/remount test passes. Baseline, Pet-only, and Git-only profiles pass load, route, diagnostics, and cleanup checks through real system Chrome CDP `9333`; the combined REAL gate proves the complete ordered tool-name list equals the `base + web-app` baseline and is the final evidence for visible Pet and Git Graph controls.
...
Fusion keeps Better Sidebar unmounted and retains the exact Pet and Git Graph `0.2.9` rows. The Task 28 report is `.superpowers/sdd/round5-task28-report.md`.
