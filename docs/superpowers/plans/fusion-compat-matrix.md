...
Checked through: Task 31 fresh registry response window `2026-08-23T00:56:11Z` through `2026-08-23T00:56:17Z`; Task 35 runtime evidence through `2026-08-23T05:16:28Z`
...
Current result: the Fusion Web external set contains exact Pet `0.2.9`. Its package-level admission remains PASS, and the current assembled one-row composition passes system Chrome 151/CDP `9333` built acceptance 1/1, the full Web driver 39/39, and the runtime-final oracle 50/50. Console, page, network, and cleanup diagnostics are clean, and pre/post target, listener, process, port, and temporary-directory checks leave no residue. Git Graph `0.2.9` remains blocked; Task 22's two-row admission and Task 29's 36/36 two-row regression remain superseded historical evidence. Task 26 rejects ModLens `3.24.0` at server-side request safety. Task 28 rejects Better Sidebar `0.15.2` at the public rc.5 peer closure; security, lifecycle, isolated installation, composition, boot, and Chrome are `NOT RUN`. Historical zero-row, three-row, four-row, and six-row results also remain superseded evidence. Task 35 does not affect TUI, so TUI is `NOT RUN (not affected)` and public delivery remains phase 2 BLOCKED.

Task 31 checked 20 packages, including all 17 Web UI identities, through the response window above and found zero post-cutoff candidates. No artifact, license, security, lifecycle, ownership, deduplication, closure, installation, composition, boot, Chrome, or PTY check was triggered, so existing candidate decisions remain unchanged.
...
Compatibility requires all applicable checks in this order: artifact identity and integrity, license identity, security, dependency closure, exact isolated installation, profile composition, actual boot, target capability and diagnostics, complete resource and effect ownership with quiescent disposal, and disconnect remounting. The first failure stops the candidate and every later check is `NOT RUN`; installation does not imply boot, and boot does not imply complete admission.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| current one-row Web composition | Pet `0.2.9`; package admission PASS below | PASS: exact identity, integrity, license, and authorization | PASS: fresh fixture-local exact install | PASS: exactly one `pet` row | PASS: built acceptance 1/1 and full driver 39/39 | PASS: Chrome 151/CDP `9333`, runtime-final 50/50, clean diagnostics and cleanup | **PASS** |
| `@liustack/modlens` | none accepted; latest audited `3.24.0` | PASS artifact/license and dependency closure; FAIL server-side request safety | PASS at `3.24.0` | PASS: one exact `modlens` row | `NOT RUN` after security failure | `NOT RUN`: cross-site `/modlens/paste` writes a file; lifecycle and Chrome stop at the first failure | **BLOCKED** |
| `@linxin666/dsh-ssh` | none accepted; latest audited `0.2.9` | PASS license and security; FAIL lifecycle | `NOT RUN` after lifecycle failure | NOT ACCEPTED: row removed | `NOT RUN` | BLOCKED: active standalone terminal sessions remain outside `SshEngine.dispose()` | **BLOCKED** |
| `@linxin666/dsh-remote-web-ui` | none accepted; latest audited `0.2.9` | PASS license; FAIL security | `NOT RUN` after security failure | NOT ACCEPTED: row removed | `NOT RUN` | BLOCKED: `requirePairingForLan:false` skips live device authorization on `/remote` | **BLOCKED** |
| `@linxin666/dsh-client-ui-task-board` | none accepted; latest audited `0.2.9` | `0.2.8` FAIL license identity; `0.2.9` FAIL lifecycle | `NOT RUN` after lifecycle failure | NOT ACCEPTED: row removed | `NOT RUN` | BLOCKED: the top-level settings subscription disposer is discarded | **BLOCKED** |
...
| `@linxin666/dsh-pet` | accepted `0.2.9` | PASS exact license and authorization | PASS exact isolated install | PASS: one `pet` row | PASS | PASS: live state, one root/dock, clean diagnostics | **PASS** |
| `@linxin666/dsh-client-ui-git-graph` | none accepted; latest audited `0.2.9` | PASS exact license, authorization, and earlier direct route/SSE checks; FAIL active-operation lifecycle | Historical exact isolated install PASS | NOT ACCEPTED: row removed | Historical isolated boot PASS | BLOCKED: active JSON request and Git child process outlive row-fiber disposal | **BLOCKED** |
| `@linxin666/dsh-client-ui-skin-center` | none accepted; latest audited `0.2.9` | FAIL license identity | `NOT RUN` | NOT ACCEPTED | `NOT RUN` | `NOT RUN` after license failure | **BLOCKED** |
| `dsh-better-sidebar` | none accepted; latest audited `0.15.2` | Artifact/license PASS; rc.5 public peer closure FAIL | `NOT RUN` after closure failure | `NOT RUN` | `NOT RUN` | Security, lifecycle, isolated installation, composition, boot, and Chrome are `NOT RUN` | **BLOCKED** |
| `@linxin666/dsh-liangshen` source | retain `0.2.4`; reject `0.2.8`/`0.2.9` | Artifact/license PASS; FAIL Windows policy | `NOT RUN` after security failure | PASS: repository preset remains sole owner | Runtime `NOT RUN` | NOT ACCEPTED: new sources retain unconfined Windows Bash | **PASS at `0.2.4`** |
...
Pet `0.2.9` package admission and the assembled one-row composition are PASS at their separate evidence levels; the other seven decision-bearing external Web capabilities remain blocked. Git Graph can be reconsidered only after a new exact release rejects new requests during disposal, cancels and awaits every active JSON/SSE operation and complete process tree within a bounded deadline, and passes an in-flight JSON unload plus same-Context remount negative control. ModLens `3.24.0` and Better Sidebar `0.15.2` stop before candidate Chrome validation, so their Chrome results are `NOT RUN`. The latest completed system Chrome CDP `9333` result is the current one-row run. TUI is `NOT RUN (not affected)` for Task 35, and public delivery remains a phase 2 blocker until a consistent rc.5 closure is publicly available or a new Harness baseline is explicitly approved and fully revalidated.

The Pet-only root HTML oracle compares every blocked `GET` with its own profile's `GET /` response snapshot, including byte-for-byte body equality, and compares every non-fallback response byte for byte across the independently booted baseline and Fusion profiles. Each root must contain exactly one parseable `window.__DSH_BOOT__` assignment: baseline has no Pet entry, Fusion adds exactly one valid Pet entry, and each graph revision derives from that graph's complete ordered entries. Removing the Pet entry and recomputing the revision from the remaining complete ordered entries must make the complete Fusion HTML byte-for-byte identical to baseline. Any additional client entry, shared-entry field or order drift, invalid graph revision on either side, body difference outside the boot script, mounted JSON, redirect, stock-title route-owned HTML, 404, or 405 control response fails the oracle.
...
- modlens `3.21.1` has a historical rc.5 boot-and-browser PASS; it is not a complete admission result under the later ordered checks.
- The retained direct web-ui `0.1.20` composition has a historical rc.5 install, boot, and browser PASS with exact React peer providers and exact `allowBuilds`; it is not a complete admission result under the later ordered checks, and the aggregate is only a fallback with duplicates.
...
These findings reduced the Task 13 Fusion Web target to zero external rows. Task 12.17 updated the bundle, fixture, tests, product documentation, desktop contract, website labels, Agent Note, and execution records. That zero-row REAL gate passed 1/1 and the complete Web oracle passed 196/196. All three negative controls blocked at 195/196 with exit 1, compact recorded seven items/401 tokens and 448 to 155 projected message tokens, restart retained 155, and the independent review recorded `EVIDENCE PASS / RUNTIME PASS`.

## Round 23: Task 18 post-cutoff audit

Round 23 audited every release after the `2026-08-21T02:11:00Z` cutoff. The independent rereview approved specification compliance and evidence quality after exact Web UI `0.2.6` and dsh-TUI `0.8.7` artifacts closed the first review's coverage gap. That round changed no admission decision: Fusion Web retained zero external rows, and Fusion TUI public delivery remained phase 2 **BLOCKED**.
...
## Round 24: Task 22 Web UI `0.2.8` and `0.2.9`

Traceable no-cache packuments were retrieved through `2026-08-22T12:37:33.085Z`; all 17 responses report `latest: 0.2.9`, and none contains a version at or above `0.2.10` as of that cutoff. The `0.2.8` wave spans `01:55:55.835Z` through `01:56:29.719Z`; the `0.2.9` wave spans `09:32:20.642Z` through `09:32:48.011Z`. All 34 exact tarballs pass package name/version identity, SHA-1, and SHA-512 SRI. Current release totals are: Chat Recovery 6, AionUI Panel 28, Community Plugins 13, Git Graph 28, Plugin Manager 8, Skill Explorer 10, Skin Center 29, Task Board 28, Web UI Settings 28, Desktop Launcher 6, Liangshen 18, Pet 28, Remote Web UI 28, Skins 30, SSH 28, Describe Image 19, and `web-ui-all` 30.

Checks run strictly in this order: identity, integrity, license, security, lifecycle, ownership, deduplication, runtime. `NOT RUN` means an earlier check failed or the identity is not selected.

| Identity | `0.2.8` first stop | `0.2.9` first stop | Runtime verdict |
| --- | --- | --- | --- |
| Chat Recovery | License FAIL: BSD manifest, Apache text | License FAIL: BSD manifest, Apache text | `NOT RUN` |
| AionUI Panel | License FAIL: Apache manifest, BSD text | Ownership FAIL: delegates the right panel to blocked Better Sidebar | `NOT RUN` |
| Community Plugins | `NOT SELECTED` after security/lifecycle PASS | `NOT SELECTED` after security/lifecycle PASS | `NOT RUN` |
| Git Graph | License FAIL: Apache manifest, BSD text | PASS through every check | Chrome PASS |
| Plugin Manager | Lifecycle FAIL: active CLI children are not cancelled or awaited on dispose | Same lifecycle failure | `NOT RUN` |
| Skill Explorer | `NOT SELECTED` after security/lifecycle PASS | `NOT SELECTED` after security/lifecycle PASS | `NOT RUN` |
| Skin Center | License FAIL: Apache manifest, BSD text | Same license failure | `NOT RUN` |
| Task Board | License FAIL: Apache manifest, BSD text | Lifecycle FAIL: top-level settings subscription disposer is discarded | `NOT RUN` |
| Web UI Settings | License FAIL: Apache manifest, BSD text | `NOT SELECTED` after security/lifecycle PASS | `NOT RUN` |
| Desktop Launcher | `NOT SELECTED` after security/lifecycle PASS | `NOT SELECTED` after security/lifecycle PASS | `NOT RUN` |
| Liangshen | Security FAIL: Windows shell explicitly lacks OS confinement | Same security failure | `NOT RUN` |
| Pet | License FAIL: Apache manifest, BSD text | PASS through every check | Chrome PASS |
| Remote Web UI | License FAIL: Apache manifest, BSD text | Security FAIL: pairing can be disabled for `/remote` | `NOT RUN` |
| Skins | License FAIL: Apache manifest, BSD text | License FAIL: dependency closure contains conflicted Skin Center `0.2.9` | `NOT RUN` |
| SSH | Lifecycle FAIL: standalone terminal sessions are not owned by engine disposal | Same lifecycle failure | `NOT RUN` |
| Describe Image | Security FAIL: non-loopback cross-site upload reaches attachment storage | Same security failure | `NOT RUN` |
| `web-ui-all` | License FAIL: Apache manifest, BSD text | License FAIL: dependency closure contains conflicted identities | `NOT RUN` |

Task 22 established that Pet and Git Graph `0.2.9` each passed unpaired, revoked, paired, and loopback authorization probes. Pet's exact client lifecycle tests passed 5/5. Git Graph's direct Host probe disposed both routes and an open SSE stream, remounted on the same registry without duplication, and its independent RED run detected retained routes, an open SSE response, active timers, and duplicate remount before forced cleanup returned all counts to zero. Its exact client apply/dispose/remount test passed. Baseline, Pet-only, and Git-only profiles passed load, route, diagnostics, and cleanup checks through real system Chrome CDP `9333`; the combined REAL gate proved the complete ordered tool-name list equaled the `base + web-app` baseline and provided historical visibility evidence. Task 33 supersedes Git Graph admission because those checks omitted an active JSON operation and child process during row-fiber disposal.

## Round 25: Task 26 external candidates

Fresh no-cache packuments report 77 ModLens releases with 39 DSH candidates and 14 installable Better Sidebar releases. Exact ModLens `3.24.0` and Better Sidebar `0.15.1` pass tarball name/version identity, registry SHA-1, SHA-512 SRI, path safety, and MIT manifest/LICENSE consistency.

Checks stop at the first failure:

| Candidate | rc.5 closure and isolated profile | Composition | Server safety | Lifecycle | Boot / Chrome CDP `9333` |
| --- | --- | --- | --- | --- | --- |
| ModLens `3.24.0` | PASS: no DSH peer/runtime dependency; exact install resolves only ModLens, Commander, and Undici | PASS: exactly one `modlens` row | FAIL: a cross-site request rejected by `/modlens/config` is accepted by `POST /modlens/paste`, returns `200`, and writes the supplied image bytes | `NOT RUN` | `NOT RUN` |
| Better Sidebar `0.15.1` | FAIL: all 14 DSH peer ranges require `^0.1.0-rc.8`, the public registry provides exact `0.1.0-rc.5` for 0/14 peer packages, and the exact-profile install stops at the missing closure | `NOT RUN` | `NOT RUN` | `NOT RUN` | `NOT RUN` |

The Better Sidebar source delta includes FileTree drag filtering, subagent auto-open debounce, Side Chat presentation, and UI-terminal parking behavior. `config.ts`, `prefs-shared.ts`, `tools.ts`, `agent-pty.ts`, and `trust-fence.ts` are byte-identical to `0.15.0`, so the artifact still contains the user-writable `agentTerminalTools` switch and ambient-environment PTY implementation; this static observation does not replace the checks skipped after the closure failure. At the Task 26 checkpoint, the Fusion manifest and two-row patch remained unchanged. Evidence is under `.superpowers/sdd/ralph-loop-3-task-26/`.

## Round 26: Task 28 Better Sidebar `0.15.2`

The fresh no-cache packument request completed at the HTTP cutoff `2026-08-22T17:01:07Z` with status `200`, `CF-Cache-Status: MISS`, and no `Age`. Its dist-tags are `latest: 0.15.2` and `beta: 0.12.0-beta.1`. The time map has 16 version keys, including the non-installable time-only `0.12.0` entry; the packument has 15 installable manifests. Relative to the prior `2026-08-22T15:28:38Z` cutoff, exact `0.15.2`, published at `2026-08-22T15:35:41.933Z`, is the sole new candidate and no higher version exists.

Exact `0.15.2` passes package name/version identity, registry SHA-1 `15c0a057d1e071e5b6c4409949fbad080f4bc6fe`, SHA-512 SRI, tar path and link safety for 218 entries, and MIT manifest/LICENSE consistency. Its 14 DSH peer ranges all remain `^0.1.0-rc.8`. Fresh no-cache requests for every peer find exact `0.1.0-rc.5` for 0/14 packages, so the public rc.5 closure fails at the first mandatory prerequisite.

Checks stop at that failure:

| Identity | Integrity | License | Public rc.5 closure | Security | Lifecycle | Isolated install | Composition / boot / Chrome CDP `9333` |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Better Sidebar `0.15.2` | PASS | PASS | FAIL: 0/14 exact rc.5 providers | `NOT RUN` | `NOT RUN` | `NOT RUN` | `NOT RUN` |

At the Task 28 checkpoint, Fusion kept Better Sidebar unmounted and retained the exact Pet and Git Graph `0.2.9` rows. Task 33 later supersedes that selected-row conclusion while preserving the Task 28 evidence. The Task 28 report is `.superpowers/sdd/round5-task28-report.md`.
