...
- **Web browser snapshot** (`pnpm run test:web`; required Linux PR gate): Chromium compares replayed output with `apps/web/tests/snapshots/`; CI fixes `DSH_SNAPSHOT=replay`, while record/refresh remain local ([web e2e lane](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md), [CI gate decision](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)). `test:web` [builds first](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md) for plugin CSS. System-Chrome lanes exercise installed `apply` routes; mutations use private copies and preserve installed hashes. Fusion pre-registers acquisitions and disposes late results. Normal cleanup is quiescent; one shared deadline reports unfinished work and preserves framework headroom. The [CI runbook](../.agents/notes/implemented/process/2026-07-26-ci-failover-runbook.md) owns the time reserves.
