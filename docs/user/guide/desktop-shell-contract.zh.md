...
桌面壳持有一个名为 `fusion` 的内部 profile。其组合包按顺序为 `@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app` 和 `@deepseek-ai/dsh-fusion`；[Fusion Web profile 指南](./fusion-profile.md)定义当前仅含 Pet 的组合。
