...
## Installing from GitHub requires prebuilt artifacts
...
`dsh plugin` forces dependency lifecycle scripts off. A Git dependency's `prepare`, `prepack`, `install`, and related hooks never run, and callers cannot override that policy. The pinned Git tree must therefore already contain every published entry point, including `lib/` for a TypeScript package; otherwise installation may succeed but the profile cannot load the package.
...
- **Publish a prebuilt Git tree** only when the pinned commit already contains the same runtime artifacts. Pin the full commit SHA so a later push cannot change what the profile loads.
