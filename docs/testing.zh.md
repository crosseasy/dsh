...
本文规定测试层级及保持绿色结果有效的规则。命令见根目录 [AGENTS.md](../AGENTS.md)；相关 Agent Note 承载设计动机。

## 层级

- **单元测试**（`pnpm run test`）：vitest 运行 `tests/**` 下的包／示例测试与 `scripts/**/*.spec.ts`；测试与所覆盖代码放在一起。每个注册表都有 HMR（热模块替换）安全覆盖：dispose（资源释放）贡献内容的 fiber，并断言清理完成。优先覆盖边界、错误、事件顺序、竞态与永久约定回归测试（见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
- **覆盖率门禁**（`pnpm run test:coverage`）：对 `packages/*/*/src` 执行必需的按文件 100% 覆盖。未覆盖代码往往应删除；行覆盖只证明代码执行过，不证明交付行为。`packages/shell/pwsh-local/src` 需要真实 `pwsh`：缺少时套件自动跳过且 `vitest.config.ts` 豁免该文件；CI 自带 pwsh 并执行 100% 门禁。
- **真实 API e2e**（`pnpm run test:e2e`）：带密钥测试调用真实提供方，包括 DeepSeek 与由提供方密钥（`EXA_API_KEY`、`PERPLEXITY_API_KEY` 等）控制的冒烟测试；缺少对应密钥时套件自动跳过，使 keyless CI 保持绿色（[真实 API e2e Agent Note](../.agents/notes/implemented/testing/2026-06-19-real-api-e2e-ci.md)）。
- **快照**（`pnpm run test:snapshot`）：无密钥输出固定传输与呈现，持久化日志固定组装后的后端行为。ACP（Agent Client Protocol）启动真实自动化服务器示例、回放会话，并对归一化 JSON-RPC 与重新持久化日志执行 diff（[ACP 快照 Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.md)）；headless 场景通过 JSONL driver 启动显式示例组装，`apps/cli` 负责产品 `dsh --profile headless` 验收。模型 transcript（文本记录）变化时使用 `pnpm run test:snapshot:record`，回放输入仍有效时使用 `pnpm run test:snapshot:refresh`；审查全部 JSONL 与预期输出差异。ACP `text-turn` 固定完整系统提示词／工具 schema 内容；其他 fixture（测试前置数据）将其 token 化以减少扰动（[pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)）。
- **Web 浏览器快照**（`pnpm run test:web`；必需的 Linux PR（Pull Request）门禁）：Chromium 将回放输出与 `apps/web/tests/snapshots/` 比较；CI 固定使用 `DSH_SNAPSHOT=replay`，record/refresh 留在本地（[web e2e 车道](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md)、[CI 门禁决策](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)）。`test:web` 会[先构建](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md)以交付插件 CSS。
- **Fusion 外部 profile 验收**（`pnpm run test:fusion:acceptance`；独立的必需 Linux PR 作业 `fusion / external-profile snapshot`）：通过已安装 route 和系统 Chrome CDP `9333` 检查 profile-local/private-copy 授权；cleanup 在一个共享截止时间内达到完全停稳（[CI 手册](../.agents/notes/implemented/process/2026-07-26-ci-failover-runbook.md)）。
